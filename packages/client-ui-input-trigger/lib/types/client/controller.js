...
export class InputTriggerController {
    ...
    /**
     * Source name or aggregate launcher key opened programmatically, or null
     * for trigger-detected/closed menus. Composer chrome subscribes to this
     * store for expanded state without owning a second menu model.
     */
     ...
     */
    headers = createSnapshotStore(new Map());
    /**
    ...
    /**
     * Toggle every source that joins the composer's plus-button launcher.
     * @param hit - synthetic hit carrying position and pick-time draft CAS.
     */
    toggleLauncher(hit) {
        if (this.disposed)
            return;
        const key = 'launcher';
        if (this.launcher.getSnapshot() === key && this.menu.getSnapshot().open) {
            this.dismiss();
            return;
        }
        const roster = this.deps.roster.sources(hit.trigger).filter(source => source.name === 'command' || source.launcher === true);
        if (roster.length === 0) {
            this.dismiss();
            return;
        }
    ...
    pickCrumb(source, index) {
        const hit = this.hit;
        if (this.disposed || !this.menu.getSnapshot().open || hit === null)
            return;
        const crumb = this.headers.getSnapshot().get(source)?.[index];
        if (crumb === undefined || crumb.current === true)
            return;
        const src = this.deps.roster.sources(hit.trigger).find(s => s.name === source);
        if (src === undefined)
            return;
    ...
    arbitrate(key, composing) {
        ...
        switch (key) {
            ...
            case 'enter': {
                ...
                if (group === undefined || group.status !== 'ready')
                    return 'consumed';
    ...
    watchLexicon(source, projection) {
        ...
        this.lexiconOffs.set(source, source.subscribeLexicon(projection, () => {
            this.refreshLexicon();
            const hit = this.hit;
            if (hit === null || !this.menu.getSnapshot().open || hit.trigger !== source.trigger)
                return;
            ...
            void Promise.resolve().then(() => {
                if (this.disposed || this.hit !== hit || !this.menu.getSnapshot().open)
                    return;
    ...
    settle(src, candidate, hit, action) {
        ...
        if (!this.execute(outcome, hit.span))
            this.drilled = false;
    }
    ...
    setHeaders(next) {
        if (this.headers.getSnapshot().size === 0 && next.size === 0)
            return;
