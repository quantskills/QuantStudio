...
export const MENU_CLOSED = { open: false, hit: null, generation: 0, groups: [], highlight: null };
/**
 ...
 */
export function seedGroups(state, sources) {
    return {
        ...state,
 ...
 */
export const menuReduce = (state, ev) => {
    switch (ev.type) {
        case 'hit': {
            if (ev.hit === null)
                return closed(state);
 ...
 */
export const exactMatch = (groups, source, name) => {
