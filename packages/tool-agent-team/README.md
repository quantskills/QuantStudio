...
`freshProvider` and `forkProvider` select registered continuable-subagent providers. `spawnToolEnabled` defaults to `true`; setting it to `false` omits only the unrestricted model-facing `spawn_teammate` tool while retaining roster, messaging, waiting, interruption, and shared-task tools. Host applications use that mode when a trusted declaration owns member identity and activation. The fixed model policy creates teammates only when the user explicitly asks for Agent Teams or teammates.

## Tools and authority

The generated [tool catalog](../../../docs/tool-catalog.md#deepseek-aidsh-tool-agent-team) owns exact schemas. The adapter supplies teammate creation; quiet and waking peer delivery; roster listing, waiting, and Lead-only interruption; and task create/list/get/compare-and-set update operations.
...
One stable policy section states the exact Team role/name/id, explicit-delegation requirement, shared-cwd behavior, filesystem stale-version recovery, Bash/formatter/codegen risk, task/write-scope coordination, quiet versus waking delivery, no-retry mailbox rule, and the Lead's duty to wait before answering. Up to ten Team schemas from `spawn_teammate` through `team_task_update` appear only in Team member scopes. A Host-controlled composition can omit `spawn_teammate` and expose its own validated member-activation tool instead.
