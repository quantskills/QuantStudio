...
export const Config = z.object({
    ...
    spawnToolEnabled: z.boolean().default(true),
});
    ...
    try {
        ...
        register(scoped.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                const hasActivePeer = ctx.agentTeams.listMembers(caller).some(member => member.id !== caller.id && ACTIVE_WAIT_STATUSES.has(member.status));
                if (!hasActivePeer) {
                    return {
                        timedOut: false,
                        noProgress: {
...
export function apply(ctx, config = {}) {
    const resolved = {
        ...
        spawnToolEnabled: config.spawnToolEnabled ?? true,
    };
    const installed = new Map();
    const maybeInstall = (agent) => {
        if (installed.has(agent) || ctx.agentTeams.tryMembership(agent) === undefined)
            return;
