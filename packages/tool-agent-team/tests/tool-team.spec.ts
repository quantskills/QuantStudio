...
describe('dsh-tool-team', () => {
  it('can withhold unrestricted teammate creation while retaining declared-Team collaboration tools', async () => {
    const { ctx, lead } = await setup(['hang'], false, { spawnToolEnabled: false })
    const schemas = (await assembly(ctx, lead)).tools.map(schema => schema.name)

    expect(schemas).not.toContain('spawn_teammate')
