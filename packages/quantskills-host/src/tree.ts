...
export function validateGitTree(
  output: string,
  declaration: QuantSkillsDeclarationFile,
  limits: GitTreeLimits,
): ValidatedGitTree {
  const records = output.split('\0')
  if (records.at(-1) !== '') invalid('QuantSkills Git tree output is incomplete.')
  records.pop()
  const entries: ValidatedGitBlob[] = []
  const pathNodes = new Map<string, { readonly original: string; readonly kind: 'directory' | 'file' }>()
  let totalBytes = 0
  for (const record of records) {
    const match = TREE_LINE.exec(record)
    if (match === null) invalid('QuantSkills Git tree contains an unsupported entry.')
    ...
    if (Buffer.byteLength(path, 'utf8') > limits.maxPathBytes) limit('QuantSkills Git tree exceeds the configured path byte limit.')
    ...
    if (!Number.isSafeInteger(bytes) || bytes < 0) invalid('QuantSkills Git tree contains an invalid blob size.')
  ...
  return Object.freeze({
    ...
    totalBytes,
    treeDigest: `sha256:${hash.digest('hex')}` as QuantSkillsTreeDigest,
  })
}
