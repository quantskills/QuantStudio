 ...
 */
export function parseQuantSkillsAgent(raw: string): ParsedQuantSkillsAgent {
  const match = /^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]+)$/.exec(raw)
  if (match === null) throw new Error('AGENTS.md requires YAML frontmatter and an instruction body')
  const parsed = frontmatterSchema.parse(parseYaml(projectFrontmatter(match[1] ?? '')) as unknown)
  ...
  return Object.freeze({
    ...
    instructions,
    requires: Object.freeze(requires.map(value => value as QuantSkillsAssetId)),
    ...(promptForm === undefined ? {} : { promptForm }),
  })
}

function projectFrontmatter(input: string): string {
  const lines = input.split(/\r?\n/)
  const projected: string[] = []
  const emittedSections = new Set<string>()
  let section: 'metadata' | 'quantSkills' | undefined
  let requirementIndent: number | undefined
  ...
  for (const line of lines) {
    ...
    if (requirementIndent !== undefined && indent > requirementIndent) projected.push(line)
  }
