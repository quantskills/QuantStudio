...
import { resolveQuantSkillsDisplayName } from './display-names.ts'
import type {
  QuantSkillsAssetId,
  QuantSkillsCatalogAsset,
  QuantSkillsCatalogCategory,
  QuantSkillsCatalogSnapshot,
  QuantSkillsCommitSha,
  QuantSkillsDeclarationFile,
  QuantSkillsSnapshotId,
} from './types.ts'

const SNAPSHOT_PATTERN = /^sha256:[a-f0-9]{64}$/
const COMMIT_PATTERN = /^[a-f0-9]{40}$/
const ASSET_PATTERN = /^(?:skill|agent)-[a-z0-9](?:[a-z0-9_-]{0,98}[a-z0-9])?$/
const OFFICIAL_CATALOG_PATTERN
  = /^https:\/\/raw\.githubusercontent\.com\/quantskills\/quantskills\/(?:main|[a-f0-9]{40})\/site\/catalog\.json$/
 ...
 */
export function validateRepositoryUrl(value: string, expectedName: string): string {
  let url: URL
  try {
    url = new URL(value)
  } catch (error) {
...
export function parseCatalogDocument(
...
): Omit<QuantSkillsCatalogSnapshot, 'refreshAfterMs' | 'sync'> {
  ...
  const assets: QuantSkillsCatalogAsset[] = []
  ...
  for (const candidate of envelope.data.assets) {
    ...
    const declaration: QuantSkillsDeclarationFile = item.project_type === 'skill' ? 'SKILL.md' : 'AGENTS.md'
    ...
    if (item.requires?.some(requirement => !ASSET_PATTERN.test(requirement)) === true) continue
    const requires = item.requires?.map(requirement => requirement as QuantSkillsAssetId)
    ...
    assets.push(Object.freeze({
      assetId: item.name as QuantSkillsAssetId,
      kind: item.project_type,
      repository,
      commit: item.commit_sha as QuantSkillsCommitSha,
      declaration,
      ...display,
      ...(title === undefined ? {} : { title }),
  ...
  return Object.freeze({
    snapshotId: envelope.data.snapshot_id as QuantSkillsSnapshotId,
    categories,
    assets: Object.freeze(assets),
  })
}

function parseCategories(input: Record<string, z.infer<typeof categorySchema>>): readonly QuantSkillsCatalogCategory[] {
 ...
 */
export async function readCatalogResponse(response: Response, maxBytes: number): Promise<unknown> {
