...
interface BrowserRequest {
  ...
  readonly socket?: { readonly remoteAddress?: string; readonly localAddress?: string; readonly localPort?: number }
}
...
function header(request: BrowserRequest, name: string): string | undefined {
...
function localAuthority(value: string | undefined): URL | undefined {
...
export function isDirectLocalBrowserRequest(request: BrowserRequest): boolean {
  ...
  for (const name of ['forwarded', 'x-forwarded-for', 'x-forwarded-host', 'x-forwarded-proto']) {
    if (request.headers instanceof Headers ? request.headers.has(name) : request.headers[name] !== undefined) return false
  }
  return true
}

/** Adapt only this Connection instance, retaining DSH's Host/Origin fence and remote authentication. */
export function installLocalBrowserAccess(connection: LocalBrowserConnection): () => void {
  const prior = {
  ...
  const replacements: LocalBrowserConnection = {
  ...
  for (const key of Object.keys(replacements) as (keyof LocalBrowserConnection)[]) {
