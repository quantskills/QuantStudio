 ...
 */
export function quantSkillsReadmeUrl(
 ...
 */
export async function readQuantSkillsReadmeResponse(
  response: Response,
  maxBytes: number,
): Promise<string> {
  ...
  if (declared !== null && (!/^\d+$/.test(declared) || Number(declared) > maxBytes)) {
    throw new QuantSkillsHostError(
      'QuantSkills README exceeds the configured byte limit.',
  ...
  try {
    ...
    throw new QuantSkillsHostError(
      'QuantSkills README is not valid UTF-8.',
      'ASSET_README_INVALID',
      { cause: error },
    )
  }
