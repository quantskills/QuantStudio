...
export const OFFICIAL_APPLICATION_REPOSITORIES = Object.freeze({
...
export class QuantSkillsApplicationUpdater {
    ...
    start() {
        ...
        if (this.status.state !== 'available' || this.status.candidateCommit === undefined
            ...
            throw new QuantSkillsHostError('Check for an official QuantSkills update before preparing it.', 'APPLICATION_UPDATE_NOT_AVAILABLE');
        }
        ...
        const task = this.prepareCandidate(source, candidateVersion, candidateCommit, currentVersion, currentCommit, checkedAt, this.lifetime.signal).catch((error) => this.recordFailure(error))
            .finally(() => { this.task = undefined; });
        this.task = task;
        return Object.freeze({ accepted: 'started', status: this.status });
    }
    ...
    async inspectSource(source, signal) {
        ...
        let origin;
        try {
            origin = (await this.commands.runGit(['config', '--get', 'remote.origin.url'], repositoryRoot, signal)).trim();
        }
        ...
        const dirty = (await this.commands.runGit(['status', '--porcelain=v1', '--untracked-files=normal'], repositoryRoot, signal)).trim();
        if (dirty !== '')
            return { ...sourceVersion, currentCommit, blocked: 'APPLICATION_UPDATE_DEVELOPMENT_DIRTY' };
        if (managedSource)
            return { ...sourceVersion, currentCommit };
        await this.commands.runGit(['fetch', '--quiet', '--no-tags', OFFICIAL_APPLICATION_REPOSITORIES[source], OFFICIAL_APPLICATION_BRANCH], repositoryRoot, signal);
        ...
        const comparison = (await this.commands.runGit(['rev-list', '--left-right', '--count', 'HEAD...FETCH_HEAD'], repositoryRoot, signal)).trim().match(/^(\d+)\s+(\d+)$/u);
        if (comparison === null) {
    ...
    async readReleaseNotes(source, release, signal) {
        ...
        if (actual !== release.commit)
            throw new QuantSkillsHostError('The release changed during discovery.', 'APPLICATION_UPDATE_CHECK_FAILED');
        try {
    ...
    async readOfficialReleases(source, signal) {
        const output = await this.commands.runGit(['ls-remote', '--tags', OFFICIAL_APPLICATION_REPOSITORIES[source], 'refs/tags/v*'], this.root, signal);
        return parseOfficialReleases(output);
    }
    async readCurrentVersion() {
        if (this.state.active === undefined)
            return await readPackageVersion(this.repositoryRoot);
    ...
    async prepareCandidate(source, candidateVersion, commit, currentVersion, currentCommit, checkedAt, signal) {
        ...
        if (release === undefined) {
            throw new QuantSkillsHostError('The selected official release is no longer published.', 'APPLICATION_UPDATE_CHECK_FAILED');
        }
        if (release.version !== candidateVersion || release.commit !== commit) {
            throw new QuantSkillsHostError('The selected official release changed after the update check.', 'APPLICATION_UPDATE_CHECK_FAILED');
        }
...
function parseOfficialReleases(output) {
    ...
    for (const line of output.split(/\r?\n/u)) {
        ...
        if (commit === undefined || reference === undefined || !SHA_PATTERN.test(commit))
            continue;
...
//# sourceMappingURL=application-update.js.map