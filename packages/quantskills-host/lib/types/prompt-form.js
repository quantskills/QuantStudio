 ...
 */
export function parseQuantSkillsPromptForm(source) {
    const match = FORM_BLOCK.exec(source);
    if (match === null)
        return undefined;
    if (Buffer.byteLength(match[0], 'utf8') > MAX_FORM_BYTES)
        return invalid('qsh-form exceeds the 16 KiB limit.');
    let input;
    try {
    ...
    if (!isRecord(input))
        return invalid('qsh-form root must be an object.');
    if (input.version !== 1)
        return invalid('qsh-form version must equal 1.');
...
function adaptPromptFormV1(input) {
    if (!Array.isArray(input.fields))
        return { input, adaptations: Object.freeze([]) };
    const adaptations = [];
    let changed = false;
    const fields = input.fields.map((field) => {
        if (!isRecord(field)
            ...
            || !JSON_NUMBER_STRING.test(field.default))
            return field;
...
function parseTask(input) {
    if (input === undefined)
        return undefined;
    if (!isRecord(input))
        return 'qsh-form task must be an object.';
...
function parseFields(input) {
    ...
    if (!Array.isArray(input))
        return 'qsh-form fields must be an array.';
    if (input.length > MAX_FIELDS)
        return `qsh-form fields may contain at most ${String(MAX_FIELDS)} entries.`;
    const keys = new Set();
    const fields = [];
...
function parseField(input, index, keys) {
    ...
    if (keys.has(input.key))
        return `${path}.key duplicates ${JSON.stringify(input.key)}.`;
    keys.add(input.key);
    if (typeof input.type !== 'string' || !FIELD_TYPES.has(input.type)) {
    ...
    if (input.help !== undefined && typeof input.help !== 'string')
        return `${path}.help must be a string.`;
...
function parseOptions(type, input, path) {
    if (type !== 'select')
        return input === undefined ? undefined : `${path}.options is only valid for select fields.`;
    if (!Array.isArray(input) || input.length === 0)
        return `${path}.options must be a non-empty array.`;
    const options = [];
    for (let index = 0; index < input.length; index++) {
...
function validateTemplate(template, fields) {
    ...
    for (const tag of template.matchAll(FORM_TAG)) {
        ...
        const name = tag[2] ?? '';
        if (!allowed.has(name))
            return `qsh-form prompt_template references undeclared variable ${JSON.stringify(name)}.`;
        if (marker === '#' || marker === '^')
            stack.push(name);
        if (marker === '/' && stack.pop() !== name)
            return 'qsh-form prompt_template contains unbalanced sections.';
        last = tagEnd;
    }
