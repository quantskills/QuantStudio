...
interface ConfiguredPythonCandidate {
  readonly command: string
  readonly args: readonly string[]
}

interface ConfiguredPythonLauncher {
  readonly executable: string
  readonly args: readonly string[]
}

interface WorkerRequest {
  readonly operation: 'describe' | 'bootstrap' | 'login' | 'logout'
  ...
  readonly baseURL?: string
  readonly accountKind?: NormalizedPandaAccount['kind']
...
const WORKER_FAILURE_CODES: ReadonlySet<PandaConnectorFailureCode> = new Set([
...
function isRunnerFailure(exitCode: number, stderr: string, rules: readonly RunnerFailureRule[]): boolean {
...
export class PandaWorkerError extends Error {
   ...
   */
  constructor(readonly code: PandaConnectorFailureCode) {
...
function isWorkerResponse(value: unknown): value is PandaWorkerResponse {
  if (typeof value !== 'object' || value === null) return false
  const record = value as Record<string, unknown>
  return typeof record.ok === 'boolean'
    ...
    && (record.code === undefined
      || (typeof record.code === 'string' && WORKER_FAILURE_CODES.has(record.code as PandaConnectorFailureCode)))
}

function privatePython(runtimeRoot: string): string {
  return process.platform === 'win32'
    ? join(runtimeRoot, 'Scripts', 'python.exe')
    : join(runtimeRoot, 'bin', 'python')
}

function virtualEnvironmentPython(root: string): string {
  return process.platform === 'win32'
    ? join(root, 'Scripts', 'python.exe')
    : join(root, 'bin', 'python')
}

function configuredPythonCandidates(config: PandaWorkerConfig): readonly ConfiguredPythonCandidate[] {
  const candidates: ConfiguredPythonCandidate[] = []
...
function operationFailure(operation: WorkerRequest['operation']): PandaConnectorFailureCode {
...
export class PandaWorkerClient {
  private readonly workerPath = fileURLToPath(new URL('../worker/panda_worker.py', import.meta.url))
  private readonly runnerPath = fileURLToPath(new URL('../worker/panda_runner.py', import.meta.url))
  private configuredPython: Promise<ConfiguredPythonLauncher> | undefined
  private managedPython: Promise<string> | undefined
   ...
   */
  constructor(private readonly subprocess: PandaProcessRuntime, private readonly config: PandaWorkerConfig) {}
  ...
  async describeAt(
    runtimeRoot: string,
    sdkVersion: string,
    pythonSource: PandaPythonSource,
    signal?: AbortSignal,
  ): Promise<PandaWorkerResponse> {
    ...
    return this.run(
      { operation: 'describe', requiredSdkVersion: sdkVersion, runtimeRoot, pythonSource },
      launcher.executable,
      launcher.args,
      signal,
    )
  ...
  async bootstrapAt(
    ...
    try {
      ...
      if (!(error instanceof PandaWorkerError)
        || (error.code !== 'python-unavailable' && error.code !== 'python-unsupported')) throw error
  ...
  loginAt(
    ...
    password: string,
    validationCall?: PandaSdkValidationCall,
    signal?: AbortSignal,
  ): Promise<PandaWorkerResponse> {
    return this.run({
  ...
  async executeAt(
    runtimeRoot: string,
    sdkVersion: string,
    request: PandaScriptExecutionRequest,
    argvWrapper: (argv: readonly string[]) => PandaExecutionArgv,
    signal?: AbortSignal,
  ): Promise<PandaScriptExecutionResult> {
  ...
  private operationSignal(caller: AbortSignal | undefined, timeoutMs: number): AbortSignal {
    const timeout = AbortSignal.timeout(timeoutMs)
    return caller === undefined ? timeout : AbortSignal.any([caller, timeout])
  }

  private async resolveConfiguredPython(signal: AbortSignal): Promise<ConfiguredPythonLauncher> {
  ...
  private async findConfiguredPython(signal: AbortSignal): Promise<ConfiguredPythonLauncher> {
  ...
  private async resolveManagedPython(signal: AbortSignal): Promise<string> {
  ...
  private async installManagedPython(signal: AbortSignal): Promise<string> {
    let uv: string
    try {
  ...
  private async runUtility(
    ...
    try {
      ...
      if (output === undefined || output.lossy) throw new PandaWorkerError('python-unavailable')
      return output.text
    } catch (error: unknown) {
      if (signal.aborted) {
        handle.terminate()
  ...
  private async run(
    ...
    try {
      ...
      const output = handle.collected.stdout?.readFrom(0)
      if (output === undefined || output.lossy) throw new PandaWorkerError('worker-failed')
      let decoded: unknown
      try {
