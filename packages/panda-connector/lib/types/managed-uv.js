...
async function boundedDownload(url, signal) {
    ...
    if (!response.ok || response.body === null)
        throw new ManagedUvError('uv download failed');
 ...
 */
export async function resolveManagedUv(managerRoot, signal) {
    const key = platformKey();
    const archive = UV_ARCHIVES[key];
    if (archive === undefined)
        throw new ManagedUvError('uv has no pinned artifact for this platform');
    ...
    if (await executableAt(finalExecutable))
    ...
    try {
        ...
        if (digest !== archive.sha256)
            throw new ManagedUvError('uv archive checksum mismatch');
        ...
        if (archive.kind === 'zip') {
            ...
            if (executable === undefined)
                throw new ManagedUvError('uv archive has no executable');
        ...
        if (!(await executableAt(stagedExecutable)))
            throw new ManagedUvError('uv archive has no regular executable');
...
//# sourceMappingURL=managed-uv.js.map