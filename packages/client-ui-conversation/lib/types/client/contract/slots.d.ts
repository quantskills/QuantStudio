...
declare module '@deepseek-ai/dsh-client-ui-slots' {
    interface SlotMap {
        ...
        /** Optional identity override around the blank-session mark and headline. */
        'conversation.hero.identity': {
            kind: 'single';
            scope: 'root';
            owner: HeroIdentityOwnerProps;
        };
        ...
        /** Compact business-owned controls after the Hero preset selector. */
        'conversation.hero.accessory': {
            kind: 'list';
            scope: 'root';
            owner: HeroAccessoryOwnerProps;
        };
...
/** Owner share of compact controls beside the Hero preset selector. */
export interface HeroAccessoryOwnerProps {
...
export interface ComposerBarInjected {
    ...
    /** Toggle the shared plus-button menu with every slash source that opts into the launcher. */
...
/** Presentation props supplied to a blank-session identity override. */
export interface HeroIdentityOwnerProps {
    /** Native mark selected through the existing brand-mark slot. */
    defaultMark: ReactNode;
    /** Native headline selected through the conversation locale. */
    defaultHeadline: string;
    /** Requested square edge for an alternative mark. */
    size: number;
    /** Host CSS class for preserving the hero mark treatment. */
    className?: string | undefined;
    /** Host formatter that preserves the native headline layout. */
    renderIdentity: (mark: ReactNode, headline: ReactNode) => ReactNode;
}
/** Full props of the resident optional-Session Conversation shell. */
export type ConversationSlotProps = PropsRuntime<'conversation'> & PropsRenderSlots<'conversation.session' | 'conversation.session.header' | 'conversation.composer' | 'conversation.composer.bar' | 'conversation.input.overlay' | 'conversation.input.dock' | 'conversation.composer.dock' | 'conversation.input.left' | 'conversation.input.right' | 'conversation.hero.brand.mark' | 'conversation.hero.identity' | 'conversation.hero.workspace' | 'conversation.hero.agentPreset' | 'conversation.hero.accessory'> & InjectFace<ConversationInjected> & PropsLocale<'conversation'>;
