...
import { FISH_LOGO_PATH, FISH_LOGO_VIEWBOX, IconChevronDownOutline14, IconFolderClose16, IconFolderOpen16, } from '@deepseek-ai/dsh-client-ui-primitives';
...
function HeroFish({ hovering }) {
    return (_jsx("svg", { className: css.fish, width: 34, height: (34 * FISH_LOGO_VIEWBOX.height) / FISH_LOGO_VIEWBOX.width, viewBox: `0 0 ${FISH_LOGO_VIEWBOX.width} ${FISH_LOGO_VIEWBOX.height}`, fill: "none", "aria-hidden": "true", children: _jsx("path", { d: FISH_LOGO_PATH, fill: "currentColor", children: hovering && (_jsx("animate", { attributeName: "d", values: `${FISH_LOGO_PATH};${HERO_SWIM_UP_PATH};${FISH_LOGO_PATH};${HERO_SWIM_DOWN_PATH};${FISH_LOGO_PATH}`, keyTimes: "0;0.35;0.55;0.75;1", calcMode: "spline", keySplines: "0.45 0 0.55 1;0.45 0 0.55 1;0.45 0 0.55 1;0.45 0 0.55 1", dur: "1.6s", repeatCount: "indefinite" })) }) }));
}
/**
...
export function HeroShell({ t, renderSlot, children }) {
    ...
    const markProps = { size: 34, className: css.fish };
    const defaultMark = renderSlot('conversation.hero.brand.mark', markProps, {
        fallback: _jsx(HeroFish, { hovering: hovering }),
    });
    const renderIdentity = (mark, headline) => _jsxs(_Fragment, { children: [_jsx("span", { className: css.fishHitbox, onMouseEnter: () => {
                    if (window.matchMedia('(hover: hover) and (prefers-reduced-motion: no-preference)').matches) {
                        setHovering(true);
                    }
                }, onMouseLeave: () => { setHovering(false); }, children: mark }), _jsx("span", { className: css.headlineText, children: headline })] });
    const defaultHeadline = t('hero.headline');
    return (_jsxs("div", { className: css.root, children: [_jsxs("div", { className: css.stack, children: [_jsxs("div", { className: css.headline, children: [renderSlot('conversation.hero.identity', {
                                defaultMark,
                                defaultHeadline,
                                ...markProps,
                                renderIdentity,
                            }, {
                                fallback: renderIdentity(defaultMark, defaultHeadline),
                            }), _jsx("span", { className: css.previewBadge, "data-hero-preview-badge": "", children: t('hero.preview') })] }), _jsx("div", { className: css.body })] }), children] }));
}
