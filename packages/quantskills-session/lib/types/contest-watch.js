...
import { prepareWatchHistory, watchHistory } from "./contest-watch-history.js";
import { parseAssessments, watchAssessments } from "./contest-watch-evaluation.js";
...
const consistentConfig = (config) => {
    ...
    if (config.builtInTemplate === 'range' && !config.rangeRules)
        return false;
    if ((config.builtInTemplate === 'trend' || config.builtInTemplate === 'breakout') && config.signalRules?.kind !== config.builtInTemplate)
        return false;
...
export function watchQuote(value, symbol, now = Date.now(), exchange) {
    ...
    if (!Number.isFinite(time))
        return { issue: '无法识别柜台行情时间格式；请检查行情接口。' };
    if (time > now + 5000)
        return { issue: '行情时间超前超过 5 秒；请检查本机时钟与柜台时间。' };
    if (now - time > 90000)
        return { issue: '行情已超过 90 秒未更新；休市时等待开盘，交易时段请检查行情连接。' };
    if (!Number.isFinite(price) || !(price > 0))
        return { issue: '柜台未返回有效最新价；等待有效报价。' };
...
export function watchAccount(snapshot, config, baseline) {
    const account = record(snapshot.account.data), equity = numeric(account.equity ?? account.totalProfit);
    if (!(equity > 0))
        throw new Error('无法确认动态权益，盯盘已暂停。');
    if (baseline !== undefined && baseline - equity >= config.maxEquityDrop)
        throw new Error('已触及权益回落停止线；请人工检查持仓。');
    if (!Array.isArray(snapshot.positions.data) || !Array.isArray(snapshot.openOrders.data))
        throw new Error('持仓或挂单数据不完整。');
    ...
    const rows = snapshot.positions.data.map(record).filter(row => {
        if (!contractParts(row.contractCode))
            throw new Error('持仓合约代码无法识别，需核对后再开始盯盘。');
        if (!sameSymbol(row.contractCode, config.symbol))
            return false;
        if (!sameSymbol(row.contractCode, config.symbol, exchange))
            throw new Error('持仓交易所与盯盘配置不符，请核对合约。');
        return true;
    });
    if (rows.length > 1)
        throw new Error('所选合约存在多条持仓，需人工处理后再开始盯盘。');
    ...
    let allowed = volume === 0 ? ['hold', 'open_long', 'open_short']
        : closable > 0 ? ['hold', row.direction === 'long' ? 'close_long' : 'close_short'] : ['hold'];
    if (config.allowedSide === 'long_only')
        allowed = allowed.filter(action => action !== 'open_short');
    if (config.allowedSide === 'short_only')
        allowed = allowed.filter(action => action !== 'open_long');
...
export function watchSpread(config, quote) {
...
export async function decideWithJev(ctx, config, samples, account, signal, request = fetch, context) {
    let key;
    try {
    ...
    if (!key)
        throw new Error('未配置 TYPESAFE_API_KEY。');
    ...
    if (!allowed.includes(answer.choice) || keys.length !== allowed.length || keys.some(key => !allowed.includes(key))
        ...
        throw new Error('Jev 返回了当前账户不允许的动作或无效概率。');
...
export class ContestWatcher {
    ...
    async templates() { await this.templateWriting.catch(() => { }); return this.readTemplates(); }
    saveTemplate(input) {
        const operation = this.templateWriting.catch(() => { }).then(async () => {
            ...
            if (!parsed.success)
                throw new Error('请填写模板名称、策略目标和有效参数；空白策略须补齐五种动作标准，品种、交易所与 tick 须一致。');
            ...
            if (item.config.autoHistory)
                delete item.config.history;
            const next = [...items.filter(value => value.name !== item.name), item];
            if (next.length > 20)
                throw new Error('最多保存 20 个模板；使用已有名称可更新该模板。');
    ...
    state = { running: false, message: '尚未启动盯盘。', sampleCount: 0, planCount: 0, events: [] };
    loading;
    ...
    async configure(input) {
        ...
        if (this.state.running || this.starting || this.stopping || this.decisionTask || this.configuring)
            throw new Error('请先停止盯盘并等待当前操作结束，再配置 Jev。');
    ...
    async start(input) {
        ...
        if (parsed.data.builtInTemplate === 'rb-range' && (!/^rb\d{4}$/i.test(parsed.data.symbol) || (parsed.data.autoHistory && parsed.data.autoHistory.exchange !== 'SHF')))
            throw new Error('内置区间模板适用于螺纹钢 rb 合约。其他品种请在微调中核对交易所、tick 与成本，并保存为自己的模板。');
        this.starting = true;
        ...
        try {
            ...
            if (config.autoHistory) {
                try {
                    config.history = await prepareWatchHistory(this.ctx, { symbol: `${config.symbol.toUpperCase()}.${config.autoHistory.exchange}`, barSeconds: config.autoHistory.barSeconds }, controller.signal);
                }
                catch (error) {
    ...
    async tick() {
        if (!this.state.running || this.working)
            return;
        clearTimeout(this.timer);
        this.working = true;
        ...
        try {
            ...
            if (Date.now() < (this.state.nextRetryAt ?? 0))
                return;
            ...
            if (unresolved) {
                if (unresolved.status === 'unknown')
                    throw new Error('存在待核实回执，盯盘已暂停；请勿重复下单。');
                this.sampleGeneration++;
            ...
            const data = await this.contest.query({ kind: 'quote', symbol: config.symbol }, identity, signal);
            check();
            if (Date.now() < (this.state.nextRetryAt ?? 0))
                return;
            this.retryCount = 0;
            ...
            if (this.state.running && this.state.runId === runId && !signal.aborted)
                this.schedule(Math.max(0, (this.state.nextRetryAt ?? started + config.intervalSeconds * 1000) - Date.now()));
        }
    }
    async analyze(account, samples, generation) {
        ...
        const check = () => { signal.throwIfAborted(); if (!this.state.running || this.state.runId !== runId)
            throw new Error('盯盘已停止。'); };
        const analysis = { id: randomUUID(), startedAt: Date.now(), sampleCount: samples.length,
        ...
        try {
            if (!this.historyCache || Date.now() - this.historyCache.at >= (this.historyCache.value.issue ? 30000 : 60000)) {
                ...
                try {
                    if (config.autoHistory && !config.history)
                        config.history = await prepareWatchHistory(this.ctx, { symbol: `${config.symbol.toUpperCase()}.${config.autoHistory.exchange}`, barSeconds: config.autoHistory.barSeconds }, signal);
            ...
            const decision = await this.decide(this.ctx, config, samples, { ...account, allowed: evidence.allowedActions }, signal, auditedJevFetch(this.contest.root, 'watch'), { evidence, history, root: this.contest.root });
            check();
            ...
            if (decision.action === 'hold' && Object.keys(decision.probabilities).length === 1) {
                ...
                finish(`受限观望 · 置信度不适用：${reason}；本轮只有观望可选，未生成交易计划。`);
                return;
            }
            ...
            const latest = watchQuote((await this.contest.query({ kind: 'quote', symbol: config.symbol }, identity, signal)).data, config.symbol, Date.now(), config.instrument?.exchange ?? config.autoHistory?.exchange);
            check();
            this.state.lastQuoteCheckedAt = Date.now();
            if (!latest.quote) {
                this.state.phase = 'waiting_quote';
            ...
            if (generation !== this.sampleGeneration || Date.now() - decision.time > 60000 || Date.now() >= this.state.expiresAt) {
            ...
            if (!verified.allowedActions.includes(decision.action)) {
            ...
            if (opening)
                this.state.openingPlanCount = (this.state.openingPlanCount ?? 0) + 1;
            ...
            if (this.state.phase === 'checking' || this.state.phase === 'deciding')
                this.state.phase = 'sampling';
            await this.save();
        }
    }
    dispose() { this.controller.abort(); clearTimeout(this.timer); }
}
//# sourceMappingURL=contest-watch.js.map