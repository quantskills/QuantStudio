...
const recommendation = (name, roles, summary, source, options = { exactId: true, lifecycle: 'stable' }) => ({
 ...
 */
export function isKimiCodeEndpoint(api, baseURL) {
...
export function suggestedRouteReasoning(api, baseURL, ids) {
    return isKimiCodeEndpoint(api, baseURL) && ids.some(id => KIMI_CODE_IDS.has(id)) ? 'high' : undefined;
}
export function suggestedModelProfile(service, api, id, baseURL) {
    ...
    if (isKimiCodeEndpoint(api, baseURL) && (id === 'k3' || id === 'k3-256k'))
        return {
            id,
    ...
    if (isKimiCodeEndpoint(api, baseURL) && (id === 'kimi-for-coding' || id === 'kimi-for-coding-highspeed'))
        return {
            id,
    ...
    if (service === 'minimax' && api === 'anthropic-messages' && id === 'MiniMax-M3')
        return {
            id,
            input: ['text', 'image'],
    ...
    if (service === 'minimax' && api === 'anthropic-messages' && /^MiniMax-M2(?:\.\d+)?(?:-highspeed)?$/.test(id))
        return {
            id,
            input: ['text'],
...
//# sourceMappingURL=model-service-catalog.js.map