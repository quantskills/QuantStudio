...
export class FlyRuntime {
            ...
            void (async () => {
                ...
                while (!this.lifetime.signal.aborted) {
                    ...
                    if (environment?.progress?.status === 'error')
                        throw new Error(environment.progress.message || '神经环境准备失败');
                    if (environment?.progress?.status === 'running')
                        this.message = `正在准备${environment.progress.stage || '神经环境'}…`;
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }
            })().catch(error => { if (!this.lifetime.signal.aborted)
                this.message = `环境准备失败：${error instanceof Error ? error.message : '请检查网络后重试'}`; })
    ...
    async process(argv) {
        ...
        await new Promise((resolve, reject) => {
            ...
            child.once('exit', code => code === 0 ? resolve() : reject(new Error('依赖准备失败')));
        });
    }
    async prepare() {
        ...
        if (!data || createHash('sha256').update(data).digest('hex') !== manifest.python.sha256) {
            ...
            if (!response.ok)
                throw new Error('Python 下载失败');
            data = Buffer.from(await response.arrayBuffer());
            if (createHash('sha256').update(data).digest('hex') !== manifest.python.sha256)
                throw new Error('Python 校验失败');
...
//# sourceMappingURL=fly-runtime.js.map
