...
const empty = () => ({ since: Date.now(), requests: 0, responsesOk: 0, unknownUsage: 0, inputTokens: 0, outputTokens: 0, records: [] });
...
export async function jevUsage(root) {
...
async function append(root, entry) {
    const writing = (queues.get(root) ?? Promise.resolve()).catch(() => { }).then(async () => {
        ...
        usage.requests++;
        if (entry.httpStatus && entry.httpStatus >= 200 && entry.httpStatus < 300)
            usage.responsesOk++;
        if (entry.usage) {
    ...
    try {
        ...
        if (queues.get(root) === writing)
            queues.delete(root);
    }
}
export function auditedJevFetch(root, purpose, request = fetch) {
    return async (url, init) => {
        const key = new Headers(init?.headers).get('Authorization')?.replace(/^Bearer /, '') ?? '';
        const entry = { id: randomUUID(), purpose, startedAt: Date.now(), finishedAt: Date.now(),
        ...
        try {
            ...
            try {
                ...
                if (typeof body.model === 'string' && /^[a-zA-Z0-9._-]{1,80}$/.test(body.model) && body.model !== key)
                    entry.model = body.model;
                const usage = jevUsageSchema.safeParse(body.usage);
                if (response.ok && usage.success)
                    entry.usage = usage.data;
            }
...
//# sourceMappingURL=contest-jev-audit.js.map