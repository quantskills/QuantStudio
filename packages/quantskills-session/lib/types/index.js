    ...
    return class QuantSkillsSessionService extends _classSuper {
        ...
        async residentSkillAttach(request, signal) {
            ...
            return this.withSessionLock(this.residentSkillTails, request.sessionId, async () => {
                ...
                const agent = this.ctx.agents.get(request.sessionId);
                if (agent === undefined)
                    throw new Error(`会话「${request.sessionId}」尚未打开，请先打开会话再加载技能。`);
                const resolved = await this.ctx.quantSkillsHost.resolveInstalledSkill(request.versionId, active);
                active.throwIfAborted();
        ...
        async promptFormList(request, signal) {
            ...
            const agent = this.ctx.agents.get(request.sessionId);
            if (agent === undefined)
                throw new Error(`QuantSkills Session "${request.sessionId}" is not live.`);
            ...
            if (!isQuantSkillsSession(agent.session.events))
                return Object.freeze({ forms: Object.freeze([]) });
        ...
        async ensureSkillSession(agent, signal) {
            ...
            if (!agent.session.header.cwd)
                throw new Error('当前会话没有工作区，请先选择工作区再加载技能。');
            ...
            try {
                ...
                if (this.plainReservations.get(sessionId) === reservation)
                    this.plainReservations.delete(sessionId);
            }
        }
