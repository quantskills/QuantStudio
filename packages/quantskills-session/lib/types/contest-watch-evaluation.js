...
export function parseAssessments(value) {
    ...
    if (!parsed.success)
        throw new Error('Jev 分项判断缺失或无效。');
    for (const name of ['regime', 'fit', 'blocker']) {
        ...
        if (!options.includes(result.choice) || keys.length !== options.length || keys.some(key => !options.includes(key))
            ...
            || result.probabilities[result.choice] < Math.max(...Object.values(result.probabilities)))
            throw new Error('Jev 分项判断选项或概率无效。');
    }
    return parsed.data;
}
//# sourceMappingURL=contest-watch-evaluation.js.map