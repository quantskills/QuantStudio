...
import { foldQuantSkillsAgentTeamMemberSession, foldQuantSkillsAgentTeamSession, foldQuantSkillsAgentSession, foldQuantSkillsAuthoringCommitted, foldQuantSkillsAuthoringStarted, foldQuantSkillsPandaRuntimeBinding, foldQuantSkillsPlainSessionBinding, foldQuantSkillsResidentSkills, foldQuantSkillsSessionBinding, parseQuantSkillsAgentTeamMemberSession, parseQuantSkillsAgentTeamSession, parseQuantSkillsAgentSession, parseQuantSkillsAuthoringCommitted, parseQuantSkillsPandaRuntimeBinding, parseQuantSkillsPlainSessionBinding, parseQuantSkillsSessionBinding, } from "./index.js";
...
const install = Object.assign((ctx, fail) => {
    const validateSession = (session) => {
        try {
            ...
            if (baseKinds > 1)
                fail('session cannot mix plain, Skill, Agent, or Agent Team QuantSkills bindings');
            if (authoring !== null && agent === null)
                fail('QuantSkills authoring purpose requires an Agent composition');
        }
        catch (error) {
    ...
    ctx.on('internal/dispatch', (_mode, eventName, args) => {
        ...
        if (event.type !== 'quantskills/plain-session'
            ...
            && event.type !== 'quantskills/authoring-committed')
            return;
        try {
            if (event.type === 'quantskills/plain-session') {
