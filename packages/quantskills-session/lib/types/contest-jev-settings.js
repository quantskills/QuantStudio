...
import { z } from 'zod';
import { readTranslator, saveTranslator, translationModels } from "./contest-jev-english.js";
const ref = credentialRef('TYPESAFE_API_KEY');
const model = 'jev-1.13.0';
export async function jevSettings(ctx, root) {
...
export async function configureJev(ctx, input, request = fetch, root) {
    ...
    if (!parsed.success)
        throw new Error('请填写有效的 Jev API Key，不含空格或换行。');
    if (parsed.data.translator) {
        if (!root || parsed.data.apiKey)
            throw new Error('请单独保存翻译模型。');
    ...
    if (!provider)
        throw new Error('本机凭据服务不可用。');
    if (parsed.data.apiKey && !settings.writable)
        throw new Error('当前密钥来自只读配置，请在对应环境变量中修改。');
    ...
    if (!key)
        throw new Error('请先填写 Jev API Key。');
    ...
    if (!response.ok)
        throw new Error(`Jev 测试失败（HTTP ${response.status}），请检查密钥、额度或稍后重试。`);
    let body;
    try {
    ...
    if (!result.success)
        throw new Error('Jev 未返回有效的连接测试结果。');
...
//# sourceMappingURL=contest-jev-settings.js.map