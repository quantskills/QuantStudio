...
export class FlyService {
    ...
    async identity(expected, verify = true) {
        ...
        if (!status.enabled || status.phase !== 'connected' || !status.identity)
    ...
    async prepare(input, signal) {
        ...
        if (opening && held && Number(held.volume ?? held.position) > 0)
            throw new Error('果蝇不加仓或直接反手。');
        if (!opening && (!held || !['long', 'short'].includes(String(held.direction))))
        ...
        if (opening && peak - equity >= limits.loss)
        ...
        if (!Number.isInteger(volume) || volume < 1 || volume > 500)
            throw new Error('名义额度不足一手或手数超过上限。');
        if (opening) {
            let occupied = 0;
