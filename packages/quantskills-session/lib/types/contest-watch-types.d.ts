...
export interface ContestWatchConfig {
    ...
    instrument?: {
        product: string;
        exchange: 'SHF' | 'DCE' | 'CZC' | 'CFE' | 'INE' | 'GFE';
        tickSize: number;
    } | undefined;
...
export interface ContestWatchTemplate {
    name: string;
...
export interface ContestWatchDecision {
    ...
    assessments?: Record<'regime' | 'fit' | 'blocker', {
        choice: string;
        confidence: number;
        probabilities: Record<string, number>;
    }>;
}
export interface ContestWatchBar {
    time: number;
    open: number;
    high: number;
    low: number;
...
export interface ContestWatchHistory {
    source: string;
    fetchedAt?: number;
    barSeconds: number;
    bars: ContestWatchBar[];
    issue?: string;
    warning?: string;
    diagnostic?: {
        stage: string;
...
export interface ContestWatchCheck {
    id: string;
    label: string;
...
export interface ContestWatchDataset {
    id: string;
    name: string;
    columns: string[];
    rows: number;
...
export interface ContestWatchQuote {
    time: number;
    price: number;
    bid?: number;
    ask?: number;
