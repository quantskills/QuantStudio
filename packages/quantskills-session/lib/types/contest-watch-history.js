...
export function historyFailure(error, stage) {
    ...
    const [code, retryable, help] = /401|403|unauthori|forbidden|登录|权限|认证|token.*expir/i.test(text) ? ['AUTH_REQUIRED', false, '请检查 PandaData 登录与数据权限。']
        : /timeout|timed?out|ETIMEDOUT|超时/i.test(text) ? ['TIMEOUT', true, '数据源响应超时，将自动重试。']
            : /429|rate.limit|频率|限流/i.test(text) ? ['RATE_LIMIT', true, '数据源限流，将稍后重试。']
                : /network|fetch failed|ECONN|ENOTFOUND|offline|网络|连接失败/i.test(text) ? ['NETWORK', true, '网络或数据源暂时不可用，将自动重试。']
                    : /column|preview|truncat|table|malformed|字段|列|表格|截断/i.test(text) ? ['INVALID_DATA', false, '请检查数据源返回内容与列映射。']
                        : ['READ_FAILED', true, '原因未明确，将自动重试；请在数据库页检查该数据源。'];
    return { issue: `历史数据读取失败（${stage}/${code}）；${help}`, diagnostic: { stage, code, retryable } };
}
export async function watchDatasets(ctx) {
...
export async function prepareWatchHistory(ctx, input, signal) {
    ...
    const gateway = ctx.get('pandaMcp');
    if (!gateway)
        throw new Error('PandaData 服务不可用，请在设置中连接。');
    const day = (time) => new Date(time + 8 * 3600000).toISOString().slice(0, 10).replaceAll('-', '');
    try {
...
export async function watchHistory(ctx, config, signal) {
    const source = config.history;
    if (!source)
        return { source: '未选择历史数据集', bars: [], barSeconds: 60, issue: '未配置历史 K 线；报价快照不能替代完整 K 线。' };
    const empty = { source: source.datasetId, bars: [], barSeconds: source.barSeconds };
    let stage = 'catalog';
    try {
        const gateway = ctx.get('pandaMcp');
        if (!gateway)
            return { ...empty, issue: '数据库服务不可用。' };
        const datasets = await gateway.databaseList(signal), dataset = datasets.find(item => item.id === source.datasetId);
        if (!dataset || dataset.kind !== 'timeseries' || dataset.category !== 'market')
            return { ...empty, issue: '所选行情数据集不存在或不是行情时间序列。' };
        if (dataset.source.kind === 'pandadata' && dataset.source.method === 'get_future_min'
            && dataset.source.params?.frequency !== (source.barSeconds === 60 ? '1m' : '5m'))
            return { ...empty, issue: '所选周期与 PandaData 数据源不一致，请重新创建对应周期的数据源。' };
        if (!Object.values(source.columns).every(column => dataset.columns.includes(column)))
            return { ...empty, issue: '数据列已变化，请重新核对列映射。' };
        ...
        if (result.status === 'insufficient')
            return { ...empty, issue: '历史缓存过期或覆盖不足；请在数据库页刷新，或允许刷新所选数据源。' };
        if (result.total > 5000)
            return { ...empty, issue: '历史数据集超过 5000 行，请配置仅包含近期目标合约的分钟数据集。' };
        let bars;
        ...
        const mapping = config.autoHistory && dataset.source.kind === 'pandadata' && dataset.source.method === 'get_future_min' ? { ...source, timeMeaning: 'close' } : source;
        try {
...
//# sourceMappingURL=contest-watch-history.js.map