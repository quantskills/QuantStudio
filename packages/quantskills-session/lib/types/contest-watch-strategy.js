export const defaultActionCriteria = {
...
export function upgradeWatchStrategy(config) {
    if (config.decisionMode !== 'jev' || config.customStrategy || config.signalRules?.kind !== 'trend' || config.instructions !== legacyTrend.instructions
        || Object.entries(legacyTrend.actionCriteria).some(([key, text]) => config.actionCriteria?.[key] !== text))
        return config;
    return { ...config,
...
export function watchStrategyWarnings(config) {
...
//# sourceMappingURL=contest-watch-strategy.js.map