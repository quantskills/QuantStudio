...
var FactorContestService = class {
	...
	async verify(expected) {
		const identity = {
			accountId: await this.call((s) => this.runtime.identity(s)),
	...
	async confirm(planId, sessionId) {
		return this.exclusive(async () => {
			...
			await this.save();
			if (p.status === "completed" && this.state.enabled && this.ready) try {
				await this.inspectCurrent();
			} catch {
...
let QuantSkillsSessionService = (() => {
	...
	return class QuantSkillsSessionService extends _classSuper {
		...
		*/
		...
		async factorQuery(request) {
			return this.factorContest.query(request);
		}
