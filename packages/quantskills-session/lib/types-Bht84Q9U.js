		...
		*/
		...
		async resultFileResponse(request) {
			...
			try {
				...
				const bytes = await streamResultBytes(this.ctx.fs, target, active);
				const responseBytes = range === void 0 ? bytes : sliceAsyncIterableBytes(bytes, range.start, range.end);
				return new Response(asyncIterableByteStream(responseBytes), {
					status,
					headers
				});
			} catch (_error) {
...
async function streamResultBytes(fs, target, signal) {
	const streaming = fs;
	if (streaming.streamBytes) return streaming.streamBytes(target, signal);
