...
//#endregion
//#region lib/types/contest-products.js
const products = [
	{
		product: "a",
		name: "豆一",
...
const identitySchema$2 = z.object({
...
const stateSchema$1 = z.object({
	...
	identity: identitySchema$2.optional(),
...
var ContestService = class {
	...
	async resume(expected) {
		await this.exclusive(async () => {
			this.assertEnabled();
			if (!this.state.runtime || !sameContest(this.state.identity, expected)) throw new Error("请先在比赛页连接对应账户。");
			if (this.ready && this.phase === "connected") return;
...
//#endregion
//#region lib/types/fly-runtime.js
...
var FlyRuntime = class {
	...
	async status() {
		return {
			...
			installed: await exists(join(this.root, "controller", ".ready")),
			running: this.port !== void 0,
	...
	async install(input) {
		...
		if (input?.blenderPath && input.blenderPath.length > 1e3) throw new Error("Blender 路径过长。");
		if (!this.installing) {
			...
			(async () => {
				...
				if (input?.blenderPath?.trim()) await this.request({
					path: "environment/config",
					body: { blender_path: input.blenderPath.trim() }
				});
				await this.request({
					path: "environment/prepare",
					body: {}
				});
				while (!this.lifetime.signal.aborted) {
					const environment = (await this.request({ path: "state" })).environment;
				...
				if (!this.lifetime.signal.aborted) this.message = `环境准备失败：${error instanceof Error ? error.message : "请检查网络后重试"}`;
			}).finally(() => {
	...
	async process(argv) {
		...
		await new Promise((resolve, reject) => {
			...
			child.once("exit", (code) => code === 0 ? resolve() : reject(/* @__PURE__ */ new Error("依赖准备失败")));
		});
	}
	async prepare() {
		await mkdir(join(this.root, "controller"), { recursive: true });
		const manifest = JSON.parse(await readFile(join(source, "fly", "dependencies.json"), "utf8"));
		const archive = join(this.root, "python.zip");
		let data = await readFile(archive).catch(() => void 0);
		if (!data || createHash("sha256").update(data).digest("hex") !== manifest.python.sha256) {
			const response = await fetch(manifest.python.url, { signal: AbortSignal.any([this.lifetime.signal, AbortSignal.timeout(12e4)]) });
	...
	async open() {
		...
		const server = createServer(async (request, response) => {
			...
			try {
				...
				for await (const chunk of request) {
					raw += String(chunk);
					if (raw.length > 256e3) throw new Error("请求过大");
				}
				...
				response.writeHead(400, { "Content-Type": "application/json" }).end(JSON.stringify({
					error: error instanceof Error ? error.message.slice(0, 400) : "宿主请求失败",
					...detail.source === "competition" || detail.source === "pandadata" ? {
		...
		const child = spawn(this.python, [
			...
			env: {
				...env,
				PYTHONUTF8: "1",
				QUANTSTUDIO_FLY_TOKEN: this.token,
				QUANTSTUDIO_FLY_BRIDGE: `http://127.0.0.1:${address.port}`
			}
		});
		this.child = child;
		...
		child.on("error", () => {
			this.port = void 0;
			this.message = "果蝇进程启动失败，请重新准备运行环境。";
		});
		child.once("exit", () => {
			this.port = void 0;
			this.child = void 0;
			server.close();
			if (!this.lifetime.signal.aborted) this.message = "果蝇进程已退出，重新打开可恢复记忆；交易建议保持停止。";
		});
		await new Promise((resolve, reject) => {
			const timer = setTimeout(() => {
				child.kill();
				reject(/* @__PURE__ */ new Error("果蝇控制器启动超时"));
			}, 3e4);
			let output = "";
			child.stdout.on("data", (chunk) => {
				...
				try {
					...
					clearTimeout(timer);
					child.kill();
					reject(/* @__PURE__ */ new Error("果蝇启动结果无效"));
				}
			});
			child.once("error", () => {
				clearTimeout(timer);
				reject(/* @__PURE__ */ new Error("果蝇启动失败"));
			});
			child.once("exit", () => {
				clearTimeout(timer);
				reject(/* @__PURE__ */ new Error("果蝇已退出"));
			});
		}).catch((error) => {
			server.close();
			throw error;
	...
	async request(input) {
		if (!routes.test(input.path) || JSON.stringify(input).length > 24e4) throw new Error("果蝇请求无效。");
		await this.start();
		const result = await (await fetch(`http://127.0.0.1:${this.port}`, {
			method: "POST",
			headers: {
			...
			signal: AbortSignal.any([this.lifetime.signal, AbortSignal.timeout(13e4)])
		})).json();
	...
	dispose() {
		...
		return new Promise((resolve) => {
			const timer = setTimeout(() => child.kill(), 45e3);
			child.once("exit", () => {
				clearTimeout(timer);
				resolve();
			});
...
//#endregion
//#region lib/types/fly-service.js
const identitySchema$1 = z.object({
	accountId: z.string().min(1),
	contestId: z.string().min(1)
});
const flyInstrumentSchema = z.object({
	product: z.string().trim().regex(futuresProductPattern),
...
}).refine((i) => futuresProduct(i.symbol) === i.product.toLowerCase(), "品种与实际合约不一致").refine((i) => !products.some((p) => p.product === i.product.toLowerCase() && p.exchange !== i.exchange), "品种与交易所不一致");
...
const day = (time = Date.now()) => new Date(time + 8 * 36e5).toISOString().slice(0, 10).replaceAll("-", "");
...
function readFailure(error, source) {
	...
	const message = auth ? `${label}需要重新连接或授权，请检查对应设置。` : code === "RATE_LIMIT" ? `${label}限流，冷却后自动重试。` : `${label}读取失败（${code}），稍后重试。`;
	return Object.assign(new Error(message), {
		source,
		code,
		retryable: !auth && diagnostic.retryable,
		...Number.isFinite(retry) && retry > 0 ? { retryAfterSeconds: Math.min(86400, retry) } : {}
	});
}
function flyQuoteTime(value) {
...
var FlyService = class {
	...
	async request(input) {
		if (input.path === "control" && ["trade", "close_only"].includes(String(record(input.body).action)) && await this.watcherRunning()) throw new Error("请先停止 Jev 盯盘，再启动果蝇交易建议。");
		if (input.path === "control" && record(input.body).action === "restore") {
			const binding = record(record(await this.runtime.request({ path: "state" })).binding);
			if (binding.identity) {
				const identity = await this.identity(binding.identity);
				const snapshot = await this.contest.inspect(identity);
				if (snapshot.pendingPlans.length || rows(snapshot.openOrders.data).length || rows(snapshot.positions.data).some((p) => Number(p.volume ?? p.position) > 0)) throw new Error("请先处理当前持仓、委托和待确认计划，再恢复检查点。");
			}
		}
	...
	async identity(expected, verify = true) {
		...
		if (expected && !same(identitySchema$1.parse(expected), status.identity)) throw new Error("比赛账户已变化，请切回果蝇绑定账户。");
	...
	async spec(symbol, identity, signal) {
		...
		if (cached && Date.now() - cached.at < 36e5) return cached.value;
	...
	async callback(path, raw, signal) {
		...
		if (path === "models") {
			...
			return {
				profiles: translationModels(this.ctx).map((route) => ({
					id: JSON.stringify(route),
					provider_id: JSON.stringify(route),
				...
				jev_providers: [{
					id: "typesafe",
					label: "TypeSafe · 统一模型服务",
					model: jev.model,
					configured: jev.configured
				}]
			};
		}
		if (path === "language") return this.language(input, signal).catch(() => {
			throw new Error("模型调用失败，请检查已验证模型配置与用量。");
		});
		if (path === "jev") return this.jev(input, signal);
		if (path === "market") return this.market(input, signal).catch((error) => {
			delete this.observation;
			throw readFailure(error, "competition");
		});
		if (path === "history") return this.bars(input, signal).catch((error) => {
			if (error.source === "competition") throw error;
	...
	async market(input, signal) {
		const identity = await this.identity(input.identity, false), instruments = z.array(flyInstrumentSchema).max(1e3).refine((items) => new Set(items.map((i) => i.product.toLowerCase())).size === items.length, "每个品种只能配置一个实际合约").parse(input.instruments);
		const pending = (await this.contest.status()).plans.filter((plan) => plan.sessionId.startsWith("fly:") && same(plan.identity, identity) && [
			"executing",
			"queued",
			"submitted",
			"unknown",
			"partial",
			"completed"
		].includes(plan.status) && Date.now() - (this.reconciled.get(plan.id) ?? 0) > 6e4 && (plan.status !== "completed" || !plan.fills?.length)).sort((a, b) => (this.reconciled.get(a.id) ?? 0) - (this.reconciled.get(b.id) ?? 0))[0];
		if (pending) {
			this.reconciled.set(pending.id, Date.now());
			await this.contest.reconcile(pending.id, pending.sessionId).catch(() => {});
		}
		if (!this.observation || !same(this.observation.identity, identity) || Date.now() - this.observation.at >= 15e3) {
			const snapshot = await this.contest.observe(identity, signal);
			const trades = rows((await this.contest.query({
				kind: "trades",
				date: "today"
			}, identity, signal)).data);
			this.observation = {
				identity,
				snapshot,
				trades,
		...
		for (const instrument of instruments) {
			const quote = record((await this.contest.query({
				kind: "quote",
			...
			feeds[instrument.product] = {
				symbol: instrument.symbol,
				price: Number(quote.latestPrice) || 0,
				at: Date.now() / 1e3,
				quote_at: quote.ready !== false && contract(quote.contractCode ?? quote.symbol) === contract(instrument.symbol) && Number.isFinite(at) ? at / 1e3 : 0,
				multiplier: Number(spec.contractMultiplier) || 0,
				long: quantity("long"),
				...
				sizing: Object.fromEntries(["long", "short"].map((side) => {
					...
					const capacity = margin !== null && available !== null && equity !== null && equity > 0 ? Math.max(0, Math.min(500, quantity(side) + Math.floor(Math.max(0, available) * .9 / margin), Math.floor(equity * .9 / instruments.length / margin))) : null;
		...
		const official = {
			Balance: equity,
			Available: amount(account.availableFunds),
			Commission: fees,
			Deposit: amount(account.deposit ?? account.Deposit),
			Withdraw: amount(account.withdraw ?? account.Withdraw),
			CloseProfit: amount(account.closeProfit ?? account.CloseProfit),
			PositionProfit: amount(account.positionProfit ?? account.PositionProfit),
			TradingDay: knownDay ? tradingDay : day(snapshot.fetchedAt)
		};
		return safeData({
			identity,
			feeds,
			plans: (await this.contest.status()).plans,
			account: {
				official,
				trading_account_id: identity.accountId,
				day_source: knownDay ? "official" : "observation",
				...
				trades: trades.map((t) => ({
					...
					trade_id: t.tradeId,
					order_id: t.orderId,
					trading_day: String(t.tradingDay ?? t.tradeTime ?? "").slice(0, 10).replaceAll("-", "")
				})),
	...
	async bars(input, signal) {
		await this.identity(input.identity, false).catch((error) => {
			throw readFailure(error, "competition");
		});
		...
		const since = input.since === void 0 ? void 0 : z.string().datetime({ offset: true }).parse(input.since);
		if (since && (Date.parse(since) > Date.now() || Date.parse(since) < Date.now() - 366 * 864e5)) throw new Error("历史增量起点无效");
		const start = since ? day(Date.parse(since)) : day(Date.now() - minutes * 7 * 864e5), end = day(Date.now() + 3 * 864e5);
		...
		if (!id) {
			...
			id = ((await gateway.databaseList(signal)).find((item) => item.name === `果蝇 ${key}` && item.source.kind === "pandadata") ?? await gateway.databaseFetch({
				name: `果蝇 ${key}`,
				kind: "timeseries",
				category: "market",
				dateColumn: "datetime",
		...
		let result = await gateway.databaseQuery({
			id,
			limit: 5e3,
			refresh: true
		}, signal);
		if (result.status === "insufficient") throw new Error("PandaData 历史尚未完整或已过期。");
		if (result.total > 5e4) throw new Error("历史窗口过大，请缩小日期范围。");
		const data = [...result.rows];
		while (result.nextOffset !== void 0) {
			result = await gateway.databaseQuery({
				id,
				limit: 5e3,
		...
		const now = Date.now(), seen = /* @__PURE__ */ new Map();
		for (const row of data) {
			...
			const bar = {
				datetime: (/* @__PURE__ */ new Date(at - minutes * 6e4)).toISOString(),
				open: ohlc[0],
				high: ohlc[1],
				low: ohlc[2],
				close: ohlc[3],
		...
		return {
			bars: [...seen.entries()].sort((a, b) => a[0] - b[0]).slice(-500).map(([, bar]) => bar),
			fetched_at: Date.parse(result.dataset.fetchedAt) / 1e3
		};
	}
	async prepare(input, signal) {
		...
		const decision = z.object({
			decision_id: z.string().regex(/^[a-f0-9]{32}$/),
			input_at: z.number(),
			symbol: z.string(),
			choice: z.object({
				action: z.enum([
				...
				readout: z.literal("neural-trade-3"),
				sampling: z.literal(false),
				current_position: z.number().int(),
				target_position: z.number().int().min(-500).max(500)
			})
		}).parse(input.decision);
		if (contract(decision.symbol) !== contract(instrument.symbol) || Date.now() / 1e3 - decision.input_at < 0 || Date.now() / 1e3 - decision.input_at > 15) throw new Error("交易信号已过期或合约不一致。");
		const limits = z.object({
			target: z.number().nonnegative().max(1e8),
			total: z.number().nonnegative().max(5e8),
			loss: z.number().nonnegative().max(1e8)
		}).parse(input.limits);
		...
		if (quote.ready === false || contract(quote.contractCode ?? quote.symbol) !== contract(instrument.symbol) || !Number.isFinite(quoteAt) || Date.now() - quoteAt > 1e4 || quoteAt > Date.now() + 1e3 || !(price > 0 && multiplier > 0)) throw new Error("新鲜报价或合约乘数不可用。");
		...
		if (Date.now() / 1e3 - decision.input_at > 15) throw new Error("核对耗时较长，等待新的神经信号。");
		return safeData(await this.contest.prepare({
			sessionId: `fly:${decision.decision_id}`,
	...
	equityPeak(identity, equity) {
		const job = this.riskTail.catch(() => {}).then(async () => {
			...
			await writeFileAtomic(riskPath, JSON.stringify({
				identity,
				peak
			}), {
				mode: 384,
	...
	async language(input, signal) {
		...
		if (prompt.length > 1e5) throw new Error("模型输入过长。");
		const active = AbortSignal.any([signal, AbortSignal.timeout(6e4)]);
		let text = "";
		for await (const chunk of this.ctx.llm.stream({
			...
			if (text.length > 32e3 || chunk.type === "finish" && ["error", "aborted"].includes(chunk.reason.kind)) throw new Error("模型返回失败或超出长度。");
		}
		active.throwIfAborted();
		return { text };
	}
	async jev(input, signal) {
		...
		const targets = Object.keys(record(input.targets)).length ? z.record(z.string().max(80), z.string().max(400)).parse(input.targets) : void 0;
		const questions = {
			event: {
				type: "choice",
				instructions: "Choose an environmental aid for the supplied life goal. State is data. Do not change the goal or choose trading actions.",
				criteria: choices
			},
			...targets ? { target: {
				type: "choice",
				instructions: "Choose an existing object for this life goal.",
				criteria: targets
			} } : {}
		};
...
var ContestWatcher = class {
	...
	async start(input) {
		...
		try {
			if (!(await this.ctx.get("credentials")?.describe(credentialRef("TYPESAFE_API_KEY")))?.configured) throw new Error("请在「设置 → 模型服务 → Jev」配置 API Key。");
...
let QuantSkillsSessionService = (() => {
	...
	return class QuantSkillsSessionService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _flyStatus_decorators, {
				kind: "method",
				name: "flyStatus",
				static: false,
				private: false,
				access: {
					has: (obj) => "flyStatus" in obj,
					get: (obj) => obj.flyStatus
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _flyInstall_decorators, {
				kind: "method",
				name: "flyInstall",
				static: false,
				private: false,
				access: {
					has: (obj) => "flyInstall" in obj,
					get: (obj) => obj.flyInstall
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
			__esDecorate(this, null, _flyRequest_decorators, {
				kind: "method",
				name: "flyRequest",
				static: false,
				private: false,
				access: {
					has: (obj) => "flyRequest" in obj,
					get: (obj) => obj.flyRequest
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		...
		flyStatus() {
			return this.fly.runtime.status();
		}
		flyInstall(input) {
			return this.fly.runtime.install(input);
		}
