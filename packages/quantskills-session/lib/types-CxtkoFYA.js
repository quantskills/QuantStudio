...
var ContestService = class {
	...
	async validateIdentity(expected) {
		...
		if (record((await this.run(["doctor"])).data).allOk !== true) {
			this.ready = false;
...
}).strict().refine((value) => value.purpose === "contest" === (value.contest !== void 0) && (value.contestConversation === void 0 || value.purpose === "contest"), "contest purpose requires its bound account");
...
let QuantSkillsSessionService = (() => {
	...
	return class QuantSkillsSessionService extends _classSuper {
		...
		*/
		...
		contestQuery(request) {
			return this.contest.query(request);
		}
		...
		*/
		async plainSessionCreate(request, signal) {
			...
			const active = this.operationSignal(signal);
			if (request.contestConversation !== void 0 && request.purpose !== "contest") throw new Error("普通会话不能设置比赛对话类型。");
		...
		installResidentRuntime(agentCtx, agent, resolved) {
			...
			try {
				disposeOutputPolicy = registerLiteralPromptSection(systemPrompt, {
					...
					order: 114,
					text: () => (foldQuantSkillsPlainSessionBinding(agent.session.events) ?? this.plainReservations.get(agent.session.id)?.binding)?.purpose === "contest" ? "比赛研究在本对话中交付。仅使用本会话已开放的工具和已附文件，不使用 Shell 或任意代码，不声称创建了未生成的文件。" : [
...
function samePlainBinding(left, right) {
	return left.purpose === right.purpose && left.contest?.accountId === right.contest?.accountId && left.contest?.contestId === right.contest?.contestId && left.contestConversation === right.contestConversation;
}
