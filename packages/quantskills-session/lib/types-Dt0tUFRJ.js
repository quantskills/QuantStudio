	...
	async prepare(input, signal) {
		...
		const decision = z.object({
			...
			choice: z.object({
				...
				readout: z.literal("neural-trade-2"),
		...
		const limits = z.object({
			target: z.number().positive().max(1e8),
			total: z.number().positive().max(5e8),
			loss: z.number().positive().max(1e8)
		}).parse(input.limits);
		...
		return safeData(await this.contest.prepare({
			...
			order: {
				symbol: instrument.symbol,
				direction: opening ? decision.choice.action === "LONG" ? "buy" : "sell" : held.direction === "long" ? "sell" : "buy",
