...
async function resolveQuantSkillsManagedPath(options = {}) {
	...
	if (platform === "win32") {
		...
		const documents = (await (options.runCommand ?? runNativeCommand)("powershell.exe", WINDOWS_DOCUMENTS_COMMAND, signal)).stdout.trim();
	...
	mutate(mutation) {
		const run = this.tail.then(async () => {
			...
			return withFileLock(path, async () => {
				const changed = mutation((await this.read(path)).agents);
...
function registerLiteralPromptSection(systemPrompt, section) {
	...
	const disposeVariable = systemPrompt.variable(variableName, () => section.text);
	let disposeSection;
	try {
...
let QuantSkillsSessionService = (() => {
	...
	return class QuantSkillsSessionService extends _classSuper {
		...
		reservations = (__runInitializers(this, _instanceExtraInitializers), /* @__PURE__ */ new Map());
		...
		*/
		async resultPreview(request, signal) {
			...
			const active = this.operationSignal(signal);
			const cwd = (await this.requireQuantSkillsSession(request.sessionId, active)).header.cwd;
			if (cwd === void 0) throw new Error("QuantSkills Session has no workspace path.");
			...
			const info = await this.ctx.fs.stat(target, active);
			if (info?.type !== "file") throw new Error("QuantSkills result preview path is not a regular file.");
			const knownBytes = info.size;
			if (knownBytes !== void 0 && knownBytes > this.maxResultPreviewBytes) return Object.freeze({
				kind: "unsupported",
				path: request.path,
				bytes: knownBytes,
				reason: `Preview limit is ${String(this.maxResultPreviewBytes)} bytes.`
			});
			const mediaType = resultPreviewMediaType(request.path);
			if (mediaType === void 0) return Object.freeze({
				kind: "unsupported",
				path: request.path,
				bytes: knownBytes ?? 0,
				reason: "No built-in preview accepts this file extension."
			});
		...
		registerAssetDraftTool(agentCtx, agent, kind) {
			...
			registerLiteralPromptSection(systemPrompt, {
				...
				text: [
					...
					"The prepare action validates only. It cannot save or install the asset; the user must confirm in the returned card."
				].join("\n")
			});
			tools.register(defineTool({
		...
		registerAgentTeamDraftTool(agentCtx, agent) {
			...
			tools.register(defineTool({
				...
				description: "List saved QuantSkills Agents or prepare a validated Agent Team draft. This tool never creates, updates, or starts a Team.",
				parameters: {
					action: {
						type: "string",
						required: true,
						enum: ["list-agents", "prepare"]
					},
					draft: {
						type: "object",
						additionalProperties: false,
				...
				execute: async (args, exec) => {
					...
					const definitions = (await this.agentStore.list()).filter((definition) => !isAuthoringAgent(definition.name));
...
//#endregion
export { parseQuantSkillsSessionBinding as C, parseQuantSkillsPlainSessionBinding as S, parseQuantSkillsAgentSession as _, QuantSkillsSessionConflictError as a, parseQuantSkillsAuthoringCommitted as b, foldQuantSkillsAgentTeamMemberSession as c, foldQuantSkillsAuthoringStarted as d, foldQuantSkillsPandaRuntimeBinding as f, foldQuantSkillsSessionFileAttachments as g, foldQuantSkillsSessionBinding as h, DEFAULT_MAX_TEXT_ATTACHMENT_BYTES as i, foldQuantSkillsAgentTeamSession as l, foldQuantSkillsResidentSkills as m, DEFAULT_MAX_RESULT_ARCHIVE_BYTES as n, QuantSkillsSessionService as o, foldQuantSkillsPlainSessionBinding as p, DEFAULT_MAX_RESULT_PREVIEW_BYTES as r, foldQuantSkillsAgentSession as s, DEFAULT_MAX_DOCUMENT_ATTACHMENT_BYTES as t, foldQuantSkillsAuthoringCommitted as u, parseQuantSkillsAgentTeamMemberSession as v, parseQuantSkillsSessionFileAttachment as w, parseQuantSkillsPandaRuntimeBinding as x, parseQuantSkillsAgentTeamSession as y };
