...
async function fixture() {
  ...
  const snapshot = { identity, fetchedAt: Date.now(), account: { data: { equity: 1000000 } }, positions: { data: [] as unknown[] }, openOrders: { data: [] as unknown[] }, pendingPlans: [] as unknown[] }
  ...
  const mock = { status: async () => state, researchIdentity: vi.fn(async () => identity), inspect: async () => snapshot, observe: vi.fn(async () => snapshot),
    ...
    contractSpec: async () => ({ data: { symbol: 'rb2610', contractMultiplier: 10 } }), prepare, execute,
  ...
  const input = { identity, instrument: { product: 'rb', symbol: 'rb2610', exchange: 'SHF' },
    ...
    decision: { decision_id: 'a'.repeat(32), input_at: Date.now() / 1000, symbol: 'rb2610', choice: { action: 'LONG', readout: 'neural-trade-2', sampling: false } } }
  return { service, mock, snapshot, state, quote, prepare, execute, watcher, call, input, ctx }
}
...
describe('fly contest bridge', () => {
  ...
  it('does not add positions or reverse a held contract', async () => {
    const f = await fixture(); f.snapshot.positions.data.push({ contractCode: 'rb2610', direction: 'long', volume: 1, closable: 1 })
    await expect(f.call('prepare', f.input)).rejects.toThrow('不加仓或直接反手')
    f.input.decision.choice.action = 'CLOSE'
    await f.call('prepare', f.input)
    expect(f.prepare).toHaveBeenCalledWith(expect.objectContaining({ order: { symbol: 'rb2610', direction: 'sell', offset: 'close', volume: 1 } }), f.input.identity, expect.any(AbortSignal))
  })
  ...
  it('does not infer missing position notional or permit zero-lot plans', async () => {
    ...
    await expect(f.call('prepare', f.input)).rejects.toThrow('不足一手')
