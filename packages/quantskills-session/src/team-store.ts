...
import { z } from 'zod'
import { quantSkillsAgentDefinitionSchema } from './agent-store.ts'
import type {
  QuantSkillsAgentDefinition,
  QuantSkillsAgentTeamDefinition,
  QuantSkillsAgentTeamId,
...
export const quantSkillsAgentTeamMemberSessionSchema = z.object({
  ...
  teamId: value.teamId as QuantSkillsAgentTeamId,
...
export class QuantSkillsAgentTeamStore {
   ...
   */
  async list(): Promise<readonly QuantSkillsAgentTeamDefinition[]> {
    const document = await this.read(await this.path())
    return Object.freeze([...document.teams].sort((left, right) =>
      right.updatedAt - left.updatedAt || left.teamId.localeCompare(right.teamId)))
  }
  ...
  private async read(path: string): Promise<AgentTeamDocument> {
    let source: string
    try {
      ...
      if (isMissing(error)) return { schemaVersion: SCHEMA_VERSION, teams: [] }
      throw error
    }
  ...
  private async write(path: string, document: AgentTeamDocument): Promise<void> {
...
export function freezeTeamDefinition(definition: {
  ...
  return Object.freeze({
    teamId: definition.teamId as QuantSkillsAgentTeamId,
...
export function freezeTeamMemberSessionBinding(input: {
  ...
  return Object.freeze({
    teamId: input.teamId as QuantSkillsAgentTeamId,
 ...
 */
export function freezeTeamModelChoice(choice: QuantSkillsAgentTeamModelChoice): QuantSkillsAgentTeamModelChoice {
...
function modelChoiceFromAgent(agent: QuantSkillsAgentDefinition): QuantSkillsAgentTeamModelChoice {
