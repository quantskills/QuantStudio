...
export interface ContestCli {
  ...
  install(runtime: string, signal: AbortSignal): Promise<{ version: string; rules: string }>
}
...
export function parseCliOutput(text: string): ContestData {
  let body: Record<string, unknown>
  try { body = record(JSON.parse(text)) } catch { throw new Error('比赛 CLI 返回了无法解析的结果，请重新检查连接。') }
  if (body.ok !== true) {
...
export class OfficialContestCli implements ContestCli {
  constructor(private readonly processes: () => SubprocessRuntime, private readonly authHome: string) {}

  private async process(argv: readonly string[], cwd: string, signal: AbortSignal, timeoutMs = 45_000): Promise<string> {
  ...
  async run(runtime: string, args: readonly string[], signal: AbortSignal): Promise<ContestData> {
  ...
  async install(runtime: string, signal: AbortSignal): Promise<{ version: string; rules: string }> {
    if (process.platform !== 'win32' && process.platform !== 'darwin') throw new Error('首版比赛模式支持 Windows 与 macOS 本机运行。')
    const publicDocument = async (path: string): Promise<string> => {
      ...
      if (text.length > 128_000) throw new Error('官方比赛接入文档超过大小限制。')
      return text
    }
    ...
    if (rules.length > 128_000 || !rules.includes('name: panda-trading') || !rules.includes(`version: ${String(spec.skillVersion)}`)) throw new Error('官方交易规则校验失败。')
