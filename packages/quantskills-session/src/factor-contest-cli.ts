 ...
 */
export function factorRuntimeDirectory(runtime: string): string {
  const absolute = resolve(runtime)
  if (process.platform !== 'win32' || absolute.length <= 100) return runtime
  return join(process.env.LOCALAPPDATA || join(homedir(), 'AppData', 'Local'), 'QuantStudio', 'factor',
    createHash('sha256').update(absolute).digest('hex').slice(0, 24))
}
...
export interface FactorRuntime {
  ...
  arena(path: string, signal: AbortSignal, mutation?: { method: 'POST' | 'DELETE'; key: string; body?: JsonValue }): Promise<JsonValue>
}
export class OfficialFactorRuntime implements FactorRuntime {
  constructor(private readonly processes: () => SubprocessRuntime, private readonly authHome: string) {}
  private configPath(): string { return join(this.authHome, 'config.json') }
  private python(runtime: string): string { return join(runtime, process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python') }
  private async process(argv: readonly string[], cwd: string, signal: AbortSignal, timeout = 60_000): Promise<string> {
  ...
  async install(runtime: string, version: string, signal: AbortSignal): Promise<string> {
    ...
    try { await this.process([python, '-m', 'venv', runtime], runtime, signal, 120_000) }
    catch (error) { signal.throwIfAborted(); throw new Error('因子 CLI 的独立 Python 环境创建失败，请检查安装目录权限和路径长度。', { cause: error }) }
  ...
  private async json(url: string, signal: AbortSignal, options: RequestInit = {}): Promise<JsonValue> {
    ...
    try { body = record(JSON.parse(text)) } catch { throw new Error('平台返回格式异常，请稍后核对状态。') }
  ...
  async login(credentials: FactorCredentials, signal: AbortSignal): Promise<string> {
  ...
  private async auth(): Promise<{ token: string; uid: string }> {
    let body: Record<string, JsonValue>
    try { body = record(JSON.parse(await readFile(this.configPath(), 'utf8'))) } catch { throw new FactorApiError('LOGIN_REQUIRED') }
  ...
  async logout(): Promise<void> { await rm(this.configPath(), { force: true }) }
  async cli(runtime: string, args: readonly string[], signal: AbortSignal): Promise<Record<string, JsonValue>> {
  ...
  async arena(path: string, signal: AbortSignal, mutation?: { method: 'POST' | 'DELETE'; key: string; body?: JsonValue }): Promise<JsonValue> {
