...
export function contestQueryArgs(input: ContestQuery): string[] {
  ...
  if (query.kind === 'open-orders') return ['orders', '--status', 'open', '--count', '200']
  const args: string[] = [query.kind]
...
export class ContestService {
  ...
  private state: State = { enabled: false, plans: [] }
  private loaded?: Promise<void>
  private tail: Promise<unknown> = Promise.resolve()
  private saving: Promise<void> = Promise.resolve()
  private controller = new AbortController()
  private phase: ContestStatus['phase'] = 'disconnected'
  private message = ''
  ...
  async status(sessionId?: string): Promise<ContestStatus> {
    await this.load()
    const plans = this.state.plans.filter(plan => (!sessionId || plan.sessionId === sessionId) && sameContest(plan.identity, this.state.identity))
      .map(plan => ({ ...plan, status: plan.status === 'prepared' && plan.expiresAt <= Date.now() ? 'expired' as const : plan.status }))
  ...
  async connect(): Promise<ContestStatus> {
    return this.exclusive(async () => {
      this.assertEnabled()
      const generation = this.generation
      try {
        if (!this.state.runtime) await this.install()
        let me: Record<string, JsonValue>
        try { me = record((await this.run(['whoami'])).data) }
        catch (error) {
          if (!(error instanceof ContestCliError) || !['login_required', 'unauthorized', 'invalid_grant', 'token_expired'].includes(error.code)) throw error
          me = { loggedIn: false }
        }
        if (me.loggedIn !== true) {
          this.phase = 'authenticating'; this.message = '请在打开的 PandaAI 官网完成登录和授权。'
        ...
        this.ready = false
        if (generation === this.generation) { this.phase = 'error'; this.message = error instanceof Error ? error.message : '比赛连接失败。' }
        throw error
      }
      return this.status()
    })
  ...
  private async install(): Promise<void> {
    this.phase = 'installing'; this.message = '正在准备官方比赛 CLI 和交易规则…'
    ...
    this.state.runtime = id; this.state.version = installed.version
  ...
  private async validateAccount(expected?: ContestIdentity, signal?: AbortSignal): Promise<ContestIdentity> {
    ...
    if (expected && !sameContest(expected, identity)) {
      this.ready = false; this.phase = 'disconnected'; delete this.lastInspection
  ...
  async resume(expected: ContestIdentity): Promise<void> {
  ...
  private async inspectAccount(identity: ContestIdentity, signal: AbortSignal | undefined, full: boolean): Promise<ContestInspection> {
    return this.exclusive(async () => {
      ...
      if (!account.data || typeof account.data !== 'object' || Array.isArray(account.data)
        || !Array.isArray(positions.data) || !Array.isArray(openOrders.data)) throw new Error('账户巡检数据不完整，请重新查询。')
  ...
  async update(): Promise<ContestStatus> {
    return this.exclusive(async () => {
      ...
      try {
        ...
        this.ready = false; this.phase = 'error'; this.message = '更新未完成，已保留原版本，请重新连接。'
  ...
  async disconnect(): Promise<ContestStatus> {
    return this.exclusive(async () => {
      this.assertEnabled()
      if (this.state.plans.some(plan => openStates.has(plan.status))) throw new Error('请先查询未完成的交易回执，再退出账户。')
      await this.run(['logout'])
      this.ready = false; this.phase = 'disconnected'; delete this.state.identity
      delete this.lastInspection
      for (const plan of this.state.plans) if (plan.status === 'prepared') plan.status = 'cancelled'
      this.message = '已退出比赛账户。'; await this.save()
      return this.status()
    })
  }
  ...
  async dismiss(id: string, sessionId: string): Promise<ContestStatus> {
    return this.exclusive(async () => {
      const plan = this.findPlan(id, sessionId)
      if (plan.status === 'prepared') { plan.status = 'cancelled'; await this.save() }
      return this.status(sessionId)
    })
  }

  private findPlan(id: string, sessionId: string): ContestPlan {
    const plan = this.state.plans.find(plan => plan.id === id && plan.sessionId === sessionId)
    if (!plan) throw new Error('没有找到此会话的交易计划。')
    return plan
  }

  async reconcile(id: string, sessionId: string): Promise<ContestPlan> {
    return this.exclusive(async () => {
      const plan = this.findPlan(id, sessionId)
      this.assertReady(plan.identity)
      if (!plan.operationId) {
