...
export class QuantSkillsLibraryStore {
  ...
  constructor(home?: string) { this.root = join(resolveDshHome(home), 'quantskills') }
  ...
  async list(): Promise<readonly LibrarySourceRecord[]> { return this.read(await this.path()) }
  private async read(path: string): Promise<readonly LibrarySourceRecord[]> {
    try {
  ...
  async put(entry: LibrarySourceRecord): Promise<void> {
