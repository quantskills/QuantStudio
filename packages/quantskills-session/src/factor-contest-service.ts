...
import { z } from 'zod'
import { record, safeData, versionAtLeast } from './contest-cli.ts'
import { sameContest } from './contest-service.ts'
import type { ContestIdentity } from './contest-types.ts'
import { FactorApiError, type FactorRuntime } from './factor-contest-cli.ts'
import { FACTOR_CONTEST_ID, type FactorBudget, type FactorCandidate, type FactorContestStatus,
  type FactorCredentials, type FactorInspection, type FactorPlan, type FactorPlanAction, type FactorPoolAction, type FactorQuery, type FactorRun } from './factor-contest-types.ts'
...
const batchSchema = z.object({ hypothesis: z.string().trim().min(1).max(2000), maxRuns: z.number().int().min(1).max(50),
  ...
  const date = (s: string) => new Date(`${s.slice(0, 4)}-${s.slice(4, 6)}-${s.slice(6)}T00:00:00Z`)
...
interface State { enabled: boolean; runtime?: string; version?: string; identity?: ContestIdentity; plans: FactorPlan[]; budgets: FactorBudget[]; runs: FactorRun[] }
const summaries: Record<FactorPlanAction['kind'], string> = { budget: '授权一批因子研究', 'create-pool': '创建比赛因子池', 'update-pool': '修改因子池设置',
...
function hash(value: JsonValue): string {
  const stable = (v: JsonValue): JsonValue => Array.isArray(v) ? v.map(stable) : v && typeof v === 'object'
    ? Object.fromEntries(Object.keys(v).sort().map(k => [k, stable(v[k]!)])) : v
  return createHash('sha256').update(JSON.stringify(stable(value))).digest('hex')
}
function poolSnapshot(value: JsonValue): JsonValue {
...
function receipt(value: JsonValue): JsonValue {
  if (JSON.stringify(value).length <= 64_000) return value
...
export class FactorContestService {
  ...
  private state: State = { enabled: false, plans: [], budgets: [], runs: [] }
  ...
  private ready = false
  private phase: FactorContestStatus['phase'] = 'disconnected'
  private message = ''
  private latestVersion?: string
  private inspection?: FactorInspection
  constructor(private readonly runtime: FactorRuntime, dshHome?: string) { this.root = join(resolveDshHome(dshHome), 'quantskills', 'factor-contest') }
  private load(): Promise<void> {
    return this.loaded ??= (async () => {
      try {
        const text = await readFile(join(this.root, 'state.json'), 'utf8')
        if (text.length > 16 * 1024 * 1024) throw new Error('因子比赛记录过大。')
        this.state = stateSchema.parse(JSON.parse(text)) as State
  ...
  private async call<T>(work: (signal: AbortSignal) => Promise<T>, signal?: AbortSignal): Promise<T> {
    if (!this.state.enabled) throw new Error('因子比赛模式已关闭。')
  ...
  private cli(args: readonly string[], signal?: AbortSignal): Promise<Record<string, JsonValue>> {
  ...
  private invalidate(): void {
    this.generation++; this.ready = false; delete this.inspection
  ...
  async status(sessionId?: string): Promise<FactorContestStatus> {
    await this.load()
    const matches = (v: { identity: ContestIdentity; sessionId: string }) => sameContest(v.identity, this.state.identity) && (!sessionId || v.sessionId === sessionId)
    return structuredClone({ enabled: this.state.enabled, phase: this.state.enabled ? this.phase : 'off', message: this.message,
      ...
      plans: this.state.plans.filter(matches).map(p => ({ ...p, status: p.status === 'prepared' && p.expiresAt <= Date.now() ? 'expired' as const : p.status })),
      budgets: this.state.budgets.filter(matches), runs: this.state.runs.filter(matches) })
  }
  async mode(enabled: boolean): Promise<FactorContestStatus> {
    if (typeof enabled !== 'boolean') throw new Error('模式开关无效。')
    await this.load()
    if (enabled !== this.state.enabled) {
      this.state.enabled = enabled; this.invalidate(); this.phase = 'disconnected'
      this.message = enabled ? '连接因子账户后开始研究。' : '已停止本应用的因子操作；平台已启动回测和参赛因子池继续运行。'
      await this.save()
    }
    return this.status()
  }
  async connect(credentials?: FactorCredentials): Promise<FactorContestStatus> {
    return this.exclusive(async () => {
      ...
      try {
        ...
        if (generation === this.generation) { this.ready = false; this.phase = 'error'; this.message = error instanceof Error ? error.message : '连接失败。' }
        throw error
      }
      return this.status()
    })
  }
  async disconnect(): Promise<FactorContestStatus> {
    await this.load(); this.invalidate(); this.phase = 'disconnected'; this.message = '已退出因子账户。'
  ...
  private async install(): Promise<void> {
    this.phase = 'installing'; this.message = '正在安装独立的 PandaAI 因子 CLI…'
    ...
    this.state.runtime = runtime; this.state.version = version; this.latestVersion = version
    await this.save()
  }
  async checkUpdate(): Promise<FactorContestStatus> {
    await this.load(); this.latestVersion = await this.call(s => this.runtime.latest(s)); return this.status()
  }
  async update(): Promise<FactorContestStatus> {
    return this.exclusive(async () => {
      if (this.state.plans.some(p => (p.status === 'prepared' && p.expiresAt > Date.now()) || ['executing', 'unknown'].includes(p.status)) || this.state.budgets.some(b => b.status === 'active')
        || this.state.runs.some(r => ['creating', 'running', 'unknown'].includes(r.status))) throw new Error('请先处理待确认操作、批次和未核实回测，再更新 CLI。')
      this.ready = false
      try { await this.install(); this.phase = 'disconnected'; this.message = 'CLI 已更新，请检查连接。' }
      catch (error) { this.phase = 'error'; this.message = '更新失败，原 CLI 版本保留；请检查连接。'; throw error }
      return this.status()
    })
  }
  ...
  private async snapshot(action: FactorPlanAction): Promise<JsonValue> {
  ...
  private validatePoolAction(action: FactorPoolAction, value: JsonValue): void {
    const p = record(value)
    if (action.kind === 'create-pool') { if (value !== null) throw new Error('已有比赛因子池，请刷新。'); return }
    id.parse(p.pool_id)
    ...
    if (action.kind === 'update-pool' && action.cycle !== undefined && p.cycle_locked !== false && action.cycle !== p.rebalance_cycle_days) throw new Error('正式提交后的调仓周期已锁定。')
    ...
    if (action.kind === 'remove-factor' || action.kind === 'replace-factor') {
      ...
      if ((p.submitted_at !== null && record(p.modification_window).open !== true)
        || factor[action.kind === 'remove-factor' ? 'can_delete' : 'can_edit'] === false) throw new Error('当前因子不可修改或删除，请等待赛事修改窗口。')
    }
  }
  async prepare(sessionId: string, input: FactorPlanAction, expected?: ContestIdentity): Promise<FactorPlan> {
    id.parse(sessionId)
    const action = actionSchema.parse(input) as FactorPlanAction
    return this.exclusive(async () => {
      this.assertReady(expected); const inspection = await this.inspectCurrent()
      ...
      if (action.kind === 'budget') {
        if (this.state.runs.some(r => sameContest(r.identity, inspection.identity) && ['creating', 'running', 'unknown'].includes(r.status))) throw new Error('请先核实未完成的回测记录。')
      ...
      if (this.state.plans.length >= 1000) throw new Error('操作记录已达上限，请整理比赛记录。')
      const snapshot = await this.snapshot(action)
      const plan: FactorPlan = { id: randomUUID(), sessionId, identity: inspection.identity, action, summary: summaries[action.kind], snapshot,
        snapshotHash: hash(snapshot), createdAt: Date.now(), expiresAt: Date.now() + 10 * 60_000, status: 'prepared' }
      this.state.plans.push(plan); await this.save(); return structuredClone(plan)
    })
  }
  async confirm(planId: string, sessionId: string): Promise<FactorPlan> {
    return this.exclusive(async () => {
      ...
      if (p.expiresAt <= Date.now()) { p.status = 'expired'; await this.save(); throw new Error('计划已过期，请重新生成。') }
      const inspection = await this.inspectCurrent()
      if (hash(await this.snapshot(p.action)) !== p.snapshotHash) { p.status = 'expired'; await this.save(); throw new Error('因子池或工作流已变化，请重新生成确认计划。') }
      if (p.action.kind === 'budget') {
        if (this.state.budgets.some(b => b.status === 'active' && sameContest(b.identity, p.identity))
          || this.state.runs.some(r => sameContest(r.identity, p.identity) && ['creating', 'running', 'unknown'].includes(r.status))) throw new Error('存在研究批次或未核实回测。')
        if (inspection.balance <= 0) throw new Error('算力余额不足。')
        ...
        p.status = 'completed'; p.result = { budgetId: p.id, message: '预算已授权；等待在本会话继续研究。' }; await this.save(); return structuredClone(p)
      }
      this.validatePoolAction(p.action, inspection.pool)
      const mutation = this.mutation(p.action, record(inspection.pool).pool_id as string)
      p.status = 'executing'; await this.save()
      try {
        ...
        p.result = { message: p.status === 'failed' ? (error as Error).message : '提交结果待核实，请查询因子池并核对，勿重复提交。' }
      }
      await this.save()
  ...
  private mutation(a: FactorPoolAction, poolId: string): { path: string; method: 'POST' | 'DELETE'; body?: JsonValue } {
    if (a.kind === 'create-pool') return { path: '/factorPool/pools', method: 'POST', body: { name: a.name, style_tag: a.style || null, rebalance_cycle_days: a.cycle } }
    const base = `/factorPool/pools/${id.parse(poolId)}`
    if (a.kind === 'update-pool') return { path: base, method: 'POST', body: { name: a.name, style_tag: a.style || null, ...(a.cycle === undefined ? {} : { rebalance_cycle_days: a.cycle }) } }
    if (a.kind === 'submit-pool') return { path: `${base}/submit`, method: 'POST' }
    if (a.kind === 'add-factor') return { path: `${base}/factors`, method: 'POST', body: { workflow_id: a.workflowId } }
  ...
  async dismiss(planId: string, sessionId: string): Promise<void> {
    return this.exclusive(async () => {
      const p = this.state.plans.find(p => p.id === planId && p.sessionId === sessionId)
      if (!p || !sameContest(p.identity, this.state.identity) || p.status !== 'prepared') throw new Error('无法取消此计划。')
      p.status = 'cancelled'; await this.save()
    })
  }
  async stopBudget(budgetId: string): Promise<void> {
    await this.load()
  ...
  async runCandidate(sessionId: string, budgetId: string, input: FactorCandidate, expected: ContestIdentity, signal?: AbortSignal): Promise<FactorRun> {
    const candidate = candidateSchema.parse(input) as FactorCandidate
    return this.exclusive(async () => {
      this.assertReady(expected); await this.verify(expected)
      const prior = this.state.runs.find(r => r.candidate.requestId === candidate.requestId && r.budgetId === budgetId && r.sessionId === sessionId)
      if (prior) { if (hash(safeData(prior.candidate)) !== hash(safeData(candidate))) throw new Error('请求编号已用于其他因子。'); return structuredClone(prior) }
      ...
      if (this.state.runs.some(r => sameContest(r.identity, expected) && ['creating', 'running', 'unknown'].includes(r.status))) throw new Error('已有未完成回测，不能重复启动。')
      ...
      b.creditsUsed += Math.max(0, b.baseline - balance); b.baseline = balance
      if (b.runsUsed >= b.maxRuns || b.creditsUsed >= b.creditThreshold || balance <= 0) {
        b.status = 'exhausted'; await this.save(); throw new Error('研究次数或算力停止阈值已到，未启动新回测。')
      }
      if (this.state.runs.length >= 2000) throw new Error('回测记录已达上限。')
      if (b.status !== 'active') throw new Error('批次已停止。')
      const r: FactorRun = { id: randomUUID(), budgetId, sessionId, identity: expected, candidate, createdAt: Date.now(), status: 'creating' }
      this.state.runs.push(r); b.runsUsed++; await this.save()
      try {
        ...
        r.workflowId = id.parse(created.factor_id); await this.save()
        if (b.status !== 'active') { r.status = 'failed'; r.result = { message: '批次已停止；仅创建工作流，没有启动回测。' }; await this.save(); return structuredClone(r) }
        r.status = 'running'; await this.save()
        ...
        b.creditsUsed += Math.max(0, b.baseline - after); b.baseline = after
      ...
      } catch { r.status = 'unknown'; b.status = 'unknown'; r.result = { message: '创建或回测结果待核实。次数已保留，停止本批次；请核对记录，勿重发。' } }
      await this.save(); return structuredClone(r)
    })
  }
  async reconcileRun(runId: string): Promise<FactorRun> {
    return this.exclusive(async () => {
      const r = this.state.runs.find(r => r.id === runId && sameContest(r.identity, this.state.identity))
      if (!r) throw new Error('回测记录不存在。')
      this.assertReady(r.identity); await this.verify(r.identity)
      ...
      if ([2, 3, 6].includes(Number(result.status))) {
        ...
        if (b) { const balance = await this.balance(); b.creditsUsed += Math.max(0, b.baseline - balance); b.baseline = balance; if (b.status === 'unknown') b.status = 'stopped' }
      }
      await this.save(); return structuredClone(r)
    })
  }
