...
import { z } from 'zod'
import { MODEL_SERVICES, suggestedModelProfile, suggestedRouteReasoning } from './model-service-catalog.ts'
import { MODEL_ACCESS_NS, type ModelAccessSettings, type ConnectionPolicy } from './model-access-settings.ts'
import type { ModelAccessRequest, ModelAccessResponse, ModelConnection, ModelConnectionDraft, ModelVerification } from './model-access-types.ts'
...
const draftSchema = z.object({
  route: routeSchema.optional(), service: z.enum(MODEL_SERVICES.map(item => item.id) as [string, ...string[]]),
...
type ModelProfile = Record<string, JsonValue>
type Profile = { apiKeyEnv?: string; catalogProvider?: string; displayName?: string; baseURL?: string; api?: string; reasoning?: string; models?: ModelProfile[]; [key: string]: unknown }
const failed = (code: string, message: string): ModelVerification => ({ state: 'failed', code, message, modelIds: [] })
const unavailable = (): ModelVerification => ({ state: 'discovery-unavailable', code: 'discovery', message: '此服务未提供可用的模型发现接口。可以填写模型 ID 保存，再用推理测试验证（可能消耗额度）。', modelIds: [] })
...
export function modelDiscoveryRequest(draft: ModelConnectionDraft, key: string): { url: string; headers: Record<string, string> } {
  ...
  if (draft.api === 'google-generative-ai') return { url: base + '/models', headers: { 'x-goog-api-key': key } }
...
export class QuantSkillsModelAccess {
  ...
  private services: Context | undefined
  private readonly proofs = new Map<string, { result: ModelVerification; expires: number }>()
  constructor(private readonly ctx: Context, _codexExecutable?: string, private readonly request: typeof fetch = fetch) {
  ...
  private profiles(): Record<string, Profile> {
    return (this.connected.settings.get(NS) as { providers?: Record<string, Profile> } | undefined)?.providers ?? {}
  }
  private get connected(): Context {
    if (!this.services) throw new Error('模型设置或凭据服务尚未就绪，请重新连接 Host。')
    return this.services
  }
  private policies(): Record<string, ConnectionPolicy> {
    return (this.connected.settings.get(MODEL_ACCESS_NS) as ModelAccessSettings | undefined)?.connections ?? {}
  }
  private fingerprint(draft: ModelConnectionDraft): string {
  ...
  private async policy(route: string, value: ConnectionPolicy): Promise<void> {
    await this.connected.settings.mutate(MODEL_ACCESS_NS, [{ op: 'set', path: ['connections', route], value }])
  }
  private async connections(): Promise<ModelConnection[]> {
    ...
    for (const [route, profile] of Object.entries(this.profiles())) {
      ...
      if (ids.length === 0) {
        try { ids = (await this.ctx.llm.listModels(route)).map(model => model.id) } catch { /* Keep saved connection manageable. */ }
      }
  ...
  private async verify(draft: ModelConnectionDraft): Promise<ModelVerification> {
    const prior = draft.route ? this.profiles()[draft.route] : undefined
    ...
    try {
      ...
      while (!seen.has(url)) {
        ...
        if ([404, 405, 501].includes(response.status)) return unavailable()
        if (!response.ok) return failed('network', '服务暂时不可用（HTTP ' + response.status + '），请稍后重试。')
        const data = await response.json() as { data?: { id?: string }[]; models?: { name?: string }[]; has_more?: boolean; last_id?: string; nextPageToken?: string }
        if (!Array.isArray(data.data) && !Array.isArray(data.models)) return failed('protocol', '返回内容不是受支持的模型目录格式；请检查地址和协议。')
        modelIds.push(...(data.data ?? []).flatMap(model => typeof model.id === 'string' ? [model.id] : []),
          ...(data.models ?? []).flatMap(model => typeof model.name === 'string' ? [model.name.replace(/^models\//, '')] : []))
      ...
      const overrides = prior?.modelOverrides as Record<string, ModelProfile> | undefined
      return { state: 'verified', code: 'ok', message: '连接与模型发现通过。目录不代表每个模型均支持对话或工具；可用推理测试单独确认。',
        modelIds: uniqueIds, modelProfiles: uniqueIds.map(id => this.modelProfile(draft, id,
          prior?.models?.find(model => model.id === id) ?? overrides?.[id])) }
    } catch { return failed('network', '连接失败或超时，请检查网络、地址和代理；未跟随重定向发送密钥。') }
  }
  /** Writes serialize across tabs so stale connection forms cannot drop other providers. */
  async run(input: ModelAccessRequest): Promise<ModelAccessResponse> {
    const run = this.tail.then(() => this.execute(input))
    this.tail = run.catch(() => {})
    return run
  }
  private async execute(input: ModelAccessRequest): Promise<ModelAccessResponse> {
    let verification: ModelVerification | undefined
    if (!['list', 'verify', 'save', 'remove', 'auto', 'test'].includes(input.action)) throw new Error('未知模型接入操作。')
    ...
    if (input.action === 'verify' || input.action === 'save') {
      ...
      if (input.action === 'remove') {
        ...
        try { await this.connected.settings.mutate(NS, [{ op: 'unset', path: ['providers', route.data] }]) }
        catch {
  ...
  private async save(draft: ModelConnectionDraft, verification: ModelVerification): Promise<void> {
    ...
    const overrides = prior?.modelOverrides as Record<string, ModelProfile> | undefined
    let models: ModelProfile[] = ids.map(id => this.modelProfile(draft, id, prior?.models?.find(model => model.id === id) ?? overrides?.[id]))
    if (draft.modelsJson !== undefined) {
      try {
        const parsed: unknown = JSON.parse(draft.modelsJson)
        ...
        models = parsed
      } catch { throw new Error('高级模型配置必须是包含有效 id 的 JSON 数组。') }
    }
    ...
    const profile: Profile = { ...prior, displayName: draft.name, baseURL: draft.baseURL, api: draft.api, models,
      ...(reasoning ? { reasoning } : {}),
  ...
  private modelProfile(draft: ModelConnectionDraft, id: string, existing?: ModelProfile): ModelProfile {
    const suggested = suggestedModelProfile(draft.service, draft.api, id, draft.baseURL)
    if (!existing) return suggested
    const next: ModelProfile = { ...suggested, ...existing, id }
    ...
    if (existing.reasoningEfforts === undefined || (typeof existing.reasoningEfforts === 'object' && existing.reasoningEfforts !== null
      ...
      if (suggested['reasoningEfforts'] !== undefined) next.reasoningEfforts = suggested['reasoningEfforts']
    }
    if (suggested['compat'] && typeof suggested['compat'] === 'object' && existing.compat && typeof existing.compat === 'object') {
      next.compat = { ...suggested['compat'] as Record<string, JsonValue>, ...existing.compat as Record<string, JsonValue> }
    }
    return next
  }
}
