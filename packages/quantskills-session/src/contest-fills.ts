...
export function planOrderId(plan: ContestPlan): string | undefined {
  if (plan.operation !== 'place_order' || ['prepared', 'cancelled'].includes(plan.status)) return undefined
  const result = plan.result ?? {}
  const ids = [record(result.latestOrder).orderId, record(result.result).orderId].filter((id): id is string => typeof id === 'string' && id.length > 0)
...
export function mergePlanFills(plan: ContestPlan, rows: unknown): boolean {
  ...
  for (const value of rows) {
    ...
    if (row.orderId !== orderId || !actual || actual[1]!.toLowerCase() !== expected[1]!.toLowerCase()
      ...
      || row.side !== parameters.side || row.offset !== parameters.offset) continue
    ...
    if (!Number.isFinite(timestamp) || timestamp < plan.createdAt - 5000 || timestamp > Date.now() + 5000) continue
