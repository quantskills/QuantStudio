...
import { z } from 'zod'
import { auditedJevFetch } from './contest-jev-audit.ts'
import { englishJevInput } from './contest-jev-english.ts'
...
const distribution = <T extends string>(keys: readonly T[]) => z.record(z.enum(keys), probability)
  .refine(values => Math.abs(Object.values(values).reduce<number>((sum, value) => sum + Number(value), 0) - 1) < 0.01)
...
export async function evaluateContestWithJev(ctx: Context, contest: ContestService, identity: ContestIdentity,
  input: { evidence: string; proposal: string }, signal: AbortSignal, request: typeof fetch = fetch) {
  ...
  if (JSON.stringify(state).length > 24000) throw new Error('Jev 评估资料过长，请缩小研究范围。')
  ...
  } catch { throw new Error(active.aborted ? 'Jev 评估已取消或超时。' : 'Jev 连接失败，请稍后重试。') }
  ...
  let body: unknown
  try { body = await response.json() } catch { throw new Error('Jev 响应读取失败，请稍后重试。') }
  active.throwIfAborted()
