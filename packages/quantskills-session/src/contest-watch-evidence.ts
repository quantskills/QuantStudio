...
export function normalizeWatchBars(rows: Record<string, unknown>[], config: NonNullable<ContestWatchConfig['history']>, symbol: string, now = Date.now()): ContestWatchBar[] {
  const { columns: c, barSeconds } = config
  const bars: ContestWatchBar[] = []
  for (const row of rows) {
    const contract = String(row[c.symbol] ?? '').replace(/\.(SHF|DCE|CZC|CFE|INE|GFE)$/i, '')
    if (contract.toLowerCase() !== symbol.toLowerCase()) continue
    ...
    if (config.timeMeaning === 'open') time += barSeconds * 1000
    if (time > now) continue // A forming/future bar is never evidence.
    const values = [c.open, c.high, c.low, c.close].map(key => typeof row[key] === 'number' ? row[key] as number : typeof row[key] === 'string' && String(row[key]).trim() ? Number(row[key]) : NaN)
...
type Account = { allowed: ContestWatchAction[]; direction: unknown; entryPrice?: number | undefined }
const show = (value: number | null) => value === null ? '未知' : Number(value.toFixed(3)).toString()

/** Close-labelled bars must touch both sides of a known intraday recess exactly. */
function historyContinuity(bars: ContestWatchBar[], barSeconds: number, config: ContestWatchConfig): { valid: boolean; detail: string } {
  ...
  for (let i = 1; i < bars.length; i++) {
    ...
    if (breaks.some(([end, restart]) => previous === midnight + end! * 60000 && current === midnight + restart! * 60000 + step)) { recesses++; continue }
    const stamp = (time: number) => new Date(time + offset).toISOString().slice(0, 19).replace('T', ' ')
...
export function watchEvidence(config: ContestWatchConfig, samples: ContestWatchQuote[], account: Account, history: ContestWatchHistory, now = Date.now()): ContestWatchEvidence {
  ...
  const lastBar = bars.at(-1), checks: ContestWatchEvidence['checks'] = []
  const opens: ContestWatchAction[] = ['open_long', 'open_short']
  const add = (id: string, label: string, passed: boolean | null, detail: string, actions: ContestWatchAction[] = opens) => checks.push({ id, label, state: passed === null ? 'unknown' : passed ? 'pass' : 'fail', detail, actions })
  ...
  const episodes = (side: 'low' | 'high') => {
    let count = 0, touching = false
    for (const bar of bars) { const hit = width !== null && (side === 'low' ? bar.low <= lower! + width * .1 : bar.high >= upper! - width * .1); if (hit && !touching) count++; touching = hit }
    return count
  }
  ...
  if (config.maxSpread) {
    ...
    add('spread', '开仓买卖价差', spread === null ? null : spread <= config.maxSpread + Math.max(Math.abs(quote!.ask!), Math.abs(quote!.bid!)) * Number.EPSILON * 8,
      `当前 ${spread === null ? '未知' : show(spread)}，上限 ${config.maxSpread}（价格单位）`, opens)
  }
  if (rules) {
    ...
    for (const [side, ratio] of [['long', longRewardCostRatio], ['short', shortRewardCostRatio]] as const) {
      add(`${side}_cost`, side === 'long' ? '多头目标空间 / 成本' : '空头目标空间 / 成本', ratio === null ? null : ratio + 1e-8 >= rules.minRewardCostRatio,
        `${show(ratio)}，要求 ≥ ${rules.minRewardCostRatio}；成本=点差+每手双边手续费与滑点 ${rules.roundTripCostTicks} tick（假设）`, [side === 'long' ? 'open_long' : 'open_short'])
    }
  ...
  if (signal) {
    ...
    if (signal.kind === 'trend') {
      const mean = (count: number, offset = 0) => bars.slice(-count - offset, offset ? -offset : undefined).reduce((sum, bar) => sum + bar.close, 0) / count
      ...
      invalidLong = fast <= slow; invalidShort = fast >= slow
      ...
      for (const check of checks.filter(item => item.id.endsWith('_trend'))) check.facts = { fastSma: historyReady ? fast : null, slowSma: historyReady ? slow : null, previousSlowSma: historyReady ? previousSlow : null }
      add('long_pullback', '回踩后恢复', historyReady && quote ? recentBars.some(bar => Math.abs(bar.low - fast) <= tolerance) && recentBars.every(bar => bar.low >= slow - tolerance) && lastBar!.close >= fast && quote.price >= fast && quote.price <= fast + tolerance : null,
        `最近 3 根 K 线回踩快均线，收盘及最新价恢复；最新价距快均线不超过 ${signal.pullbackTicks} tick`, ['open_long'])
      add('short_pullback', '反弹后转弱', historyReady && quote ? recentBars.some(bar => Math.abs(bar.high - fast) <= tolerance) && recentBars.every(bar => bar.high <= slow + tolerance) && lastBar!.close <= fast && quote.price <= fast && quote.price >= fast - tolerance : null,
        `最近 3 根 K 线反弹到快均线，收盘及最新价转弱；最新价距快均线不超过 ${signal.pullbackTicks} tick`, ['open_short'])
      ...
      add('long_breakout', '收盘突破前高', historyReady && upper !== null && quote ? lastBar!.close >= upper + buffer && quote.price >= upper + buffer : null,
        `前 ${signal.lookbackBars - 1} 根最高 ${show(upper)}；末根收盘与最新价均须高出 ${signal.bufferTicks} tick`, ['open_long'])
      add('short_breakout', '收盘跌破前低', historyReady && lower !== null && quote ? lastBar!.close <= lower - buffer && quote.price <= lower - buffer : null,
        `前 ${signal.lookbackBars - 1} 根最低 ${show(lower)}；末根收盘与最新价均须低出 ${signal.bufferTicks} tick`, ['open_short'])
      ...
      invalidLong = Boolean(quote && upper !== null && quote.price < upper); invalidShort = Boolean(quote && lower !== null && quote.price > lower)
    }
    add('cost', '目标空间 / 成本', longRewardCostRatio === null ? null : longRewardCostRatio + 1e-8 >= signal.minRewardCostRatio,
      `${show(longRewardCostRatio)}，要求 ≥ ${signal.minRewardCostRatio}；目标 ${signal.takeProfitTicks} tick /（点差 + 手续费及滑点假设 ${signal.roundTripCostTicks} tick）`)
    if (account.direction !== 'flat') {
