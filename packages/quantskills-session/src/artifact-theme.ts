...
export function renderArtifactTheme(settings: unknown): string {
  let palette: Record<string, unknown> | undefined
  try {
    const raw = (settings as { artifactPalette?: unknown } | undefined)?.artifactPalette
    if (typeof raw === 'string' && raw.length <= 2000) {
      ...
      if (value && ['light', 'dark'].includes(value.scheme) && keys.every(key => typeof value[key] === 'string' && /^#[a-f0-9]{6}$/i.test(value[key])) &&
        Array.isArray(value.series) && value.series.length === 6 && value.series.every((color: unknown) => typeof color === 'string' && /^#[a-f0-9]{6}$/i.test(color))) {
