	...
	factory: (require) => {
		...
		*/
		function ContextInjectionRow({ content, source, provenance, form, t }) {
			...
			const readableSource = provenance.label === "@deepseek-ai/dsh-system-prompt" ? "运行规则" : provenance.label === "skill-catalog" ? "可用技能" : provenance.label;
		...
		*/
		...
		function TurnStatus({ startTime, renderSlot, t }) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				role: "status",
				"aria-live": "polite",
				children: [renderSlot("conversation.chat.turnStatus", { defaultLabel: t("chat.deepDiving") }, { fallback: t("chat.deepDiving") }), showClock && (0, react_jsx_runtime.jsx)("span", {
		...
		*/
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [running && (0, react_jsx_runtime.jsx)("span", {
					...
					collapsedContent: (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						(0, react_jsx_runtime.jsx)("span", {
							className: ReasoningRow_module_css_default.friendlySummary,
							children: "展开查看分析过程"
						}),
						(0, react_jsx_runtime.jsx)("span", {
							ref: summaryRef,
		...
		const NOOP = () => {};
		//#endregion
		//#region lib/types/client/chat/final-deliverables.js
		function declaration(source, unmarked = false) {
			try {
				if (source.length > 128 * 1024) return void 0;
				const value = JSON.parse(source);
				if (!value || typeof value !== "object" || !("version" in value) || value.version !== 1 || !("items" in value) || !Array.isArray(value.items) || value.items.length > 64) return void 0;
				if (unmarked && (value.items.length === 0 || Object.keys(value).some((key) => key !== "version" && key !== "items"))) return void 0;
				const result = [];
				for (const item of value.items) {
					if (!item || typeof item !== "object" || typeof item.path !== "string" || typeof item.title !== "string" || item.title.length > 300 || item.path.length > 4096 || !["card", "interactive"].includes(item.presentation)) return void 0;
					if (unmarked && Object.keys(item).some((key) => ![
						"path",
						"title",
						"presentation"
					].includes(key))) return void 0;
					const path = item.path.replaceAll("\\", "/");
					if (!path.startsWith("output/") || path.endsWith("/") || /[\u0000-\u001f:<>|?*{}]/u.test(path) || path.split("/").some((part) => part === "" || part === "." || part === "..")) return void 0;
					if (!item.title.trim() || item.presentation === "interactive" && !/\.html?$/iu.test(path)) return void 0;
					result.push({
						path,
		...
		function projectFinalDeliverables(blocks, streaming = false, options = {}) {
			...
			return {
				blocks: blocks.map((block) => {
					...
					for (let i = 0; i < lines.length; i++) {
						...
						const compatible = opening[2].trim().toLowerCase() === "json" && marker >= 0 && output[marker].trim() === "quantskills-deliverables";
						const candidate = dedicated || compatible;
						const unmarked = !candidate && options.allowUnmarked === true && !streaming && ["", "json"].includes(opening[2].trim().toLowerCase());
						...
						} else if (!(candidate && streaming && end === lines.length)) output.push(...lines.slice(i, Math.min(end + 1, lines.length)));
						else if (compatible) output.splice(marker);
						i = end;
					}
					return {
						...block,
		...
		const AssistantNodeView = (0, react.memo)(function AssistantNodeView({ node, useTurnData, useSession, turnProcess, openFile, renderMessageImages, fileMentions, renderSlot, t }) {
			const data = node.data;
			...
			const delivery = (0, react.useMemo)(() => projectFinalDeliverables(data.blocks, data.status === "running", { allowUnmarked: owner !== void 0 && data.status === "settled" }), [
				data.blocks,
				data.status,
				owner
			]);
			const mentions = (0, react.useMemo)(() => owner === void 0 ? void 0 : fileMentions(owner), [
				data.blocks,
				openFile,
				fileMentions,
				owner
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(AssistantMarkdown, {
				...
				mentions,
				t
			}), owner !== void 0 && delivery.items.length > 0 && renderSlot("conversation.chat.deliverables", {
				items: delivery.items,
				openFile
			}, { fallback: (0, react_jsx_runtime.jsx)("ul", { children: delivery.items.map((item) => (0, react_jsx_runtime.jsx)("li", { children: (0, react_jsx_runtime.jsx)("button", {
				type: "button",
		...
		*/
		...
		const TurnTailNodeView = (0, react.memo)(function TurnTailNodeView({ node, openFile, forkAt, renderSlot, renderSlotChain, t, useChat, useSession }) {
			const data = node.data;
			const sessionId = useSession((snapshot) => snapshot.sessionId);
		...
		*/
		function registerChatNodeRenderers(ctx) {
			...
			ctx.slots.inject("conversation.chat.node", () => ctx.slots.register({
				...
				key: "assistant-step",
				locale: NS,
				children: { "conversation.chat.deliverables": {
					kind: "single",
					scope: "session"
				} }
			}, AssistantNodeView));
		...
		*/
		...
		//#endregion
		//#region ../../node_modules/@deepseek-ai/dsh-token-meter/lib/types/turn-usage.js
