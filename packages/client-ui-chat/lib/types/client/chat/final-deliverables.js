function declaration(source, unmarked = false) {
    try {
        ...
        if (!value || typeof value !== 'object' || !('version' in value) || value.version !== 1
            || !('items' in value) || !Array.isArray(value.items) || value.items.length > 64)
            return undefined;
        if (unmarked && (value.items.length === 0 || Object.keys(value).some(key => key !== 'version' && key !== 'items')))
            return undefined;
        const result = [];
        for (const item of value.items) {
            if (!item || typeof item !== 'object' || typeof item.path !== 'string' || typeof item.title !== 'string' || item.title.length > 300 || item.path.length > 4096
                || !['card', 'interactive'].includes(item.presentation))
                return undefined;
            if (unmarked && Object.keys(item).some(key => !['path', 'title', 'presentation'].includes(key)))
                return undefined;
            const path = item.path.replaceAll('\\', '/');
            if (!path.startsWith('output/') || path.endsWith('/') || /[\u0000-\u001f:<>|?*{}]/u.test(path)
                || path.split('/').some((part) => part === '' || part === '.' || part === '..'))
                return undefined;
            if (!item.title.trim() || (item.presentation === 'interactive' && !/\.html?$/iu.test(path)))
                return undefined;
...
export function projectFinalDeliverables(blocks, streaming = false, options = {}) {
    const items = new Map();
    const projected = blocks.map(block => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            while (marker >= 0 && output[marker].trim() === '')
                marker--;
...
//# sourceMappingURL=final-deliverables.js.map