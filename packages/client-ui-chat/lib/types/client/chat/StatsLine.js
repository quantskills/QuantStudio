...
import { formatCacheHitPercent, formatTokens } from "./token-format.js";
...
export const StatsLine = memo(function StatsLine({ useChat, useProjection, t }) {
    ...
    if (groups.length === 0)
        return null;
    return (_jsx(Tooltip, { label: line, side: "top", delayMs: 500, disabled: !truncated, children: _jsx("div", { ref: rootRef, className: css.root, children: groups.map((group, i) => (_jsxs(Fragment, { children: [i > 0 && _jsxs(_Fragment, { children: [_jsx("span", { className: css.sep, "aria-hidden": true, children: "|" }), ' '] }), _jsx("span", { children: group })] }, group))) }) }));
});
