...
it('keeps incomplete contract selections in onboarding instead of sending invalid settings', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '进入生活' }))
  expect((await screen.findByRole('alert')).textContent).toContain('螺纹')
  expect(screen.getByRole('textbox', { name: '螺纹实际合约' }).getAttribute('aria-invalid')).toBe('true')
  expect(calls.some(call => call.includes('/settings'))).toBe(false)
  fireEvent.change(screen.getByRole('textbox', { name: '螺纹实际合约' }), { target: { value: 'rb2610' } })
  fireEvent.click(screen.getByRole('button', { name: '下一步' }))
  fireEvent.click(screen.getByRole('button', { name: '进入生活' }))
  await waitFor(() => expect(calls.some(call => call.includes('/settings'))).toBe(true))
  expect(screen.queryByRole('dialog', { name: '果蝇首次设置' })).toBeNull()
})
...
it('fills the verified main contract and lets the user replace it with a verified delivery month', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: '3 比赛' }))
  fireEvent.click(screen.getByRole('checkbox', { name: '螺纹 · rb' }))
  await waitFor(() => expect((screen.getByRole('textbox', { name: '螺纹实际合约' }) as HTMLInputElement).value).toBe('rb2610'))
  expect(query).toHaveBeenCalledWith({ kind: 'quote', symbol: 'rb' }, expect.anything())
  fireEvent.change(screen.getByRole('combobox', { name: '螺纹合约月份' }), { target: { value: 'rb2611' } })
  await waitFor(() => expect((screen.getByRole('textbox', { name: '螺纹实际合约' }) as HTMLInputElement).value).toBe('rb2611'))
  expect(query).toHaveBeenCalledWith({ kind: 'quote', symbol: 'rb2611' }, expect.anything())
  fireEvent.change(screen.getByRole('combobox', { name: '螺纹合约月份' }), { target: { value: 'rb2612' } })
  await waitFor(() => expect(screen.getByText(/rb2612 无法确认/)).toBeTruthy())
  expect((screen.getByRole('textbox', { name: '螺纹实际合约' }) as HTMLInputElement).value).toBe('rb2611')
})
...
it('saves contract configuration separately and reuses account authorization when connecting the market once', async () => {
  ...
  render(<FlyV2Page active contest={contest}/>)
  fireEvent.click(await screen.findByRole('button', { name: '行情配置' }))
  fireEvent.click(await screen.findByRole('checkbox', { name: '螺纹 · rb' }))
