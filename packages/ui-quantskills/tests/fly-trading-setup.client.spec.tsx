...
it('places the positive trade-choice switch in environment settings', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '本次运行' }))
...
it('opens the relevant setup step from beginner guidance without saving or starting', async () => {
  ...
  await screen.findByRole('region', { name: '果蝇新手引导' })
  fireEvent.click(screen.getByRole('button', { name: '下一步说明 →' }))
  fireEvent.click(screen.getByRole('button', { name: '设置账户、合约与限额 ↗' }))
  expect(screen.getByRole('dialog', { name: '果蝇运行设置' })).toBeTruthy()
  expect(screen.getByRole('button', { name: '本次运行' }).getAttribute('aria-current')).toBe('step')
...
it('starts with the existing account and contracts without mandatory notional inputs', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '完成并开始观察' }))
...
it('keeps the trading dashboard ordered and life widgets in the life page', async () => {
  ...
  expect(screen.queryByRole('region', { name: '果蝇新手引导' })).toBeNull()
...
it('only asks for account and contracts when the environment is already ready', async () => {
  const calls = mount()
  fireEvent.click(await screen.findByRole('button', { name: '果蝇运行设置' }))
  expect(screen.getByRole('button', { name: '本次运行' }).getAttribute('aria-current')).toBe('step')
  expect(screen.queryByRole('combobox', { name: 'QuantStudio 已接入模型' })).toBeNull()
  fireEvent.click(screen.getByRole('button', { name: '模型辅助' }))
...
it('saving valid existing settings does not undo a manual stop', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: '果蝇运行设置', exact: true }))
  fireEvent.click(screen.getByRole('button', { name: '核对配置' }))
  await screen.findByText('交易配置已齐全')
...
it('discards contract and filter drafts on cancel without mutating saved settings', async () => {
  ...
  expect(screen.queryByRole('dialog')).toBeNull()
  fireEvent.click(screen.getByRole('button', { name: '果蝇运行设置' }))
