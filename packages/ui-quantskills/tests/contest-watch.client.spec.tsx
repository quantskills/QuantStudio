...
async function startFromSettings() {
  const apply = screen.queryByRole('button', { name: '应用本次配置' })
  if (apply) { fireEvent.click(apply); if (screen.queryByRole('dialog', { name: 'Jev 运行设置' })) return }
  fireEvent.click(screen.getByRole('button', { name: '开始盯盘' }))
  const confirm = screen.queryByRole('button', { name: '确认并开始盯盘' })
  if (confirm) fireEvent.click(confirm)
}
function selectTemplate(name: string) {
  const select = screen.getByLabelText('运行模板') as HTMLSelectElement
  const option = [...select.options].find(item => item.text === name)!
  fireEvent.change(select, { target: { value: option.value } })
}
describe('Jev continuous watch controls', () => {
  it('discards configuration edits when the drawer is cancelled', async () => {
    const f = fixture()
    ...
    fireEvent.change(screen.getByLabelText('手数及持仓上限'), { target: { value: '9' } })
    fireEvent.click(screen.getByRole('button', { name: '取消' }))
    fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' }))
    expect((screen.getByLabelText('实际合约') as HTMLInputElement).value).toBe('')
    expect((screen.getByLabelText('手数及持仓上限') as HTMLInputElement).value).toBe('1')
    expect(f.access.start).not.toHaveBeenCalled()
    expect(f.access.saveTemplate).not.toHaveBeenCalled()
  })

  it('requires review once for changed configuration and resumes unchanged configuration directly', async () => {
    const f = fixture()
    render(<ContestWatch access={f.access}/>); await screen.findByText('尚未启动')
    fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' }))
    ...
    expect(f.access.start).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: '开始盯盘' }))
    expect(screen.getByRole('dialog', { name: '核对本次盯盘' })).toBeTruthy()
    expect(f.access.start).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: '确认并开始盯盘' }))
    await screen.findByText('已启动')
    fireEvent.click(screen.getByRole('button', { name: '停止盯盘' }))
    await screen.findByText('已停止')
    fireEvent.click(screen.getByRole('button', { name: '开始盯盘' }))
    await waitFor(() => expect(f.access.start).toHaveBeenCalledTimes(2))
    expect(screen.queryByRole('dialog')).toBeNull()
  })
  it('guides setup and refreshes credentials without starting, configuring or preparing history', async () => {
    localStorage.clear()
    const f = fixture()
    ...
    await screen.findByText('比赛账户待连接')
    fireEvent.click(screen.getByRole('button', { name: '查看模型配置入口 ↗' }))
    await screen.findByText(/API Key 未配置。API Key/)
    fireEvent.click(screen.getByRole('button', { name: '刷新配置状态' }))
    await screen.findByText(/API Key 已配置。API Key/)
    fireEvent.click(screen.getByRole('button', { name: '关闭Jev 运行设置' }))
    fireEvent.click(screen.getByRole('button', { name: '快速上手' }))
    fireEvent.click(screen.getByRole('button', { name: '下一步说明 →' }))
    fireEvent.click(screen.getByRole('button', { name: '选择模板和实际合约 ↗' }))
    expect(screen.getByRole('dialog', { name: 'Jev 运行设置' })).toBeTruthy()
    expect(f.access.start).not.toHaveBeenCalled()
    expect(f.access.configure).not.toHaveBeenCalled()
    expect(f.access.prepareHistory).not.toHaveBeenCalled()
  })
  it('syncs and searches live products, requiring an explicit tick for new products before starting', async () => {
    ...
    render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText(/已同步柜台目录/)
    ...
    await startFromSettings(); await screen.findByText('已启动')
  ...
  it('keeps the offline catalog usable when the CLI cannot fetch metadata', async () => {
    ...
    render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText(/保留现有目录/)
  ...
  it('cancels catalog reads on unmount and does not reset manually edited parameters when metadata arrives', async () => {
    ...
    const view = render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText('尚未启动')
    fireEvent.change(screen.getByLabelText('实际合约'), { target: { value: 'm2701' } })
  ...
  it('keeps advanced settings closed while polling the visible running status', async () => {
    vi.useFakeTimers()
    const f = fixture()
    render(<ContestWatch access={f.access}/> )
    await act(async () => { await vi.advanceTimersByTimeAsync(0) })
    expect(screen.queryByRole('dialog')).toBeNull()
    expect(screen.getByRole('button', { name: '开始盯盘' })).toBeTruthy()
  ...
  it('offers a persisted opening-only cooldown and opening plan quota', async () => {
    const f = fixture(); render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText('尚未启动')
  ...
  it('points to model services without exposing a second Jev configuration form', async () => {
    ...
    render(<ContestWatch access={f.access} openModelSettings={openModelSettings}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText('尚未启动')
  ...
  it('defaults to Jev-led decisions, preserves explicit strict mode across templates and persists the choice', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '保存为我的模板' })); await screen.findByText(/已保存至本机/)
    cleanup(); render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText('尚未启动')
    selectTemplate('严格趋势')
    expect((screen.getByLabelText('决策方式') as HTMLSelectElement).value).toBe('strict')
  ...
  it('reuses the previous contract without starting automatically and retains access to the previous full configuration', async () => {
    ...
    render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText('尚未启动')
    expect((screen.getByLabelText('实际合约') as HTMLInputElement).value).toBe('rb2610')
    expect(f.access.start).not.toHaveBeenCalled()
  ...
  it('shows audited API usage and submits editable strategy settings', async () => {
    ...
    render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' })); await screen.findByText('尚未启动')
    fireEvent.click(screen.getByRole('button', { name: '关闭Jev 运行设置' }))
    fireEvent.click(screen.getByRole('button', { name: '运行记录与诊断 ↗' }))
    expect(await screen.findByText('382 / 34')).toBeTruthy()
    expect(screen.getByText(/非官网账单/)).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '关闭Jev 运行记录与诊断' }))
    fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' }))
    fireEvent.click(screen.getByText('微调模板'))
  ...
  it('starts a complete template after review and locks running configuration', async () => {
    const f = fixture(); render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' }))
    ...
    expect(screen.queryByRole('dialog', { name: '核对本次盯盘' })).toBeNull()
    await screen.findByText('已启动')
  ...
  it('directs an unconfigured user to model services before starting a watch', async () => {
    const f = fixture(), openModelSettings = vi.fn()
    vi.mocked(f.access.settings).mockResolvedValueOnce({ configured: false, writable: true, model: 'jev-1.13.0' })
    render(<ContestWatch access={f.access} connected={false} openModelSettings={openModelSettings}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' }))
  ...
  it('shows real samples, action probabilities and the recorded reason a plan was not created', async () => {
    ...
    render(<ContestWatch access={f.access}/>); fireEvent.click(screen.getByRole('button', { name: 'Jev 运行设置' }))
  ...
  it('animates the reported watch phase and stops claiming activity while status is unavailable', async () => {
    ...
    expect(within(screen.getByRole('status')).getByText(/最新行情.*价格与时间/)).toBeTruthy()
    expect(screen.getByText(/最近行情检查/)).toBeTruthy()
    expect(screen.getByRole('status').getAttribute('data-active')).toBe('true')
    vi.mocked(f.access.status).mockRejectedValueOnce(new Error('测试连接中断'))
    await act(async () => { await vi.advanceTimersByTimeAsync(3000) })
    expect(within(screen.getByRole('status')).getByText('连接中断 · 状态待确认')).toBeTruthy()
    expect(screen.getByRole('status').getAttribute('data-active')).toBe('false')
    f.setState({ running: true, phase: 'deciding', message: '分析快照', sampleCount: 8, planCount: 0, events: [] })
    await act(async () => { await vi.advanceTimersByTimeAsync(3000) })
    expect(within(screen.getByRole('status')).getByText('正在盯盘 · Jev 分析中')).toBeTruthy()
