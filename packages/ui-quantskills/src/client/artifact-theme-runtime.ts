...
export function artifactThemeRuntime(initial: ArtifactTheme) {
  const scope = window as unknown as { QuantSkillsTheme: ArtifactTheme }
  let theme = initial
  const legacy = new Map<string, { name: string; kind: 'text' | 'background'; rgb: number[] }>()
  ...
  const updateLegacy = () => {
    ...
    for (const entry of legacy.values()) {
      let rgb = entry.rgb
      if (entry.kind === 'background') rgb = rgb.map((c, i) => Math.round(c * .12 + surface[i]! * .88))
      else {
        const target = theme.scheme === 'dark' ? 255 : 0
        for (let step = 0; step <= 20; step++) {
          rgb = entry.rgb.map(c => Math.round(c + (target - c) * step / 20))
          const light = luminance(rgb)
          if ((Math.max(light, base) + .05) / (Math.min(light, base) + .05) >= 4.8) break
        }
      }
      document.documentElement.style.setProperty(entry.name, `rgb(${rgb.join(',')})`)
    }
  }
  const legacyColor = (value: string, kind: 'text' | 'background') => {
    const rgb = parse(value)
    if (!rgb) return undefined
    const key = kind + value
    let entry = legacy.get(key)
    if (!entry) { entry = { name: '--qs-report-legacy-' + legacy.size, kind, rgb }; legacy.set(key, entry) }
    return `var(${entry.name})`
  }
  const setTheme = (next: ArtifactTheme) => {
    if (!next || !['light', 'dark'].includes(next.scheme)) return
    const keys = ['background', 'surface', 'text', 'muted', 'border', 'accent', 'onAccent', 'success', 'warning', 'danger'] as const
    if (keys.some(key => !/^#[a-f0-9]{6}$/i.test(next[key])) || !Array.isArray(next.series) || next.series.length !== 6 || next.series.some(c => !/^#[a-f0-9]{6}$/i.test(c))) return
    theme = next
    scope.QuantSkillsTheme = structuredClone(next)
    document.documentElement.style.colorScheme = next.scheme
    for (const key of keys) document.documentElement.style.setProperty('--qs-report-' + key.replace('onAccent', 'on-accent'), next[key])
  ...
  window.addEventListener('message', event => {
    if (event.source === parent && event.data?.type === 'quantskills:artifact-theme') setTheme(event.data.theme)
  })
  ...
  const adapted = new WeakSet<Element>()
  const adapt = (element: Element) => {
    if (!(element instanceof HTMLElement) || adapted.has(element) || element.closest('[data-qs-theme="preserve"],svg') || ['SCRIPT', 'STYLE', 'IMG', 'VIDEO', 'CANVAS', 'IFRAME'].includes(element.tagName)) return
    adapted.add(element)
    const style = getComputedStyle(element)
    const background = style.backgroundColor, foreground = style.color, border = style.borderTopColor
    if (neutral(background)) element.style.setProperty('background-color', 'var(--qs-report-surface)', 'important')
    else if (style.backgroundImage === 'none') {
      const tint = legacyColor(background, 'background')
      if (tint) element.style.setProperty('background-color', tint, 'important')
    }
    if (neutral(foreground)) {
      ...
      if (readable) element.style.setProperty('color', readable, 'important')
    }
    if (neutral(border)) element.style.setProperty('border-color', 'var(--qs-report-border)', 'important')
  }
  const start = () => {
    ...
    new MutationObserver(records => {
      for (const record of records) for (const node of record.addedNodes) if (node instanceof Element) {
        adapt(node); node.querySelectorAll('*').forEach(adapt)
      }
      updateLegacy()
