...
import { MINIMAL_THEMES } from './minimal-themes.ts'
...
const read = (): ThemePreset => {
  try {
    const saved = localStorage.getItem(key)
    return THEME_PRESETS.find(theme => theme.id === saved)?.id ?? 'minimal-blue'
  } catch { return 'minimal-blue' }
}
let memory: ThemePreset | undefined
...
export function useThemePreset() { return useSyncExternalStore(subscribe, () => memory ?? read(), () => 'minimal-blue' as const) }
export function setThemePreset(value: ThemePreset) {
  memory = value
  try { localStorage.setItem(key, value) } catch { /* Keep usable when storage is unavailable. */ }
