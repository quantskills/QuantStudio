...
function fluid(canvas: HTMLCanvasElement, theme: Theme) {
  const gl = canvas.getContext('webgl', { alpha: false, antialias: false, powerPreference: 'low-power' })
  if (!gl) return undefined
  const shaders: WebGLShader[] = []
  const compile = (kind: number, source: string) => {
    const shader = gl.createShader(kind)
    if (!shader) throw new Error('Fluid shader unavailable')
    shaders.push(shader); gl.shaderSource(shader, source); gl.compileShader(shader)
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) throw new Error('Fluid shader unavailable')
    return shader
  }
  const program = gl.createProgram()
  if (!program) return undefined
  let buffer: WebGLBuffer | null = null
  const destroy = () => { for (const shader of shaders) gl.deleteShader(shader); gl.deleteProgram(program); if (buffer) gl.deleteBuffer(buffer) }
  try {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) throw new Error('Fluid program unavailable')
    gl.useProgram(program)
    buffer = gl.createBuffer(); gl.bindBuffer(gl.ARRAY_BUFFER, buffer)
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1,-1,3,-1,-1,3]), gl.STATIC_DRAW)
    const position = gl.getAttribLocation(program, 'position')
    gl.enableVertexAttribArray(position); gl.vertexAttribPointer(position, 2, gl.FLOAT, false, 0, 0)
    const names = ['resolution','motion','base','primary','secondary','tertiary','ink','pointer','rings[0]'] as const
    ...
    return {
      ...
      draw(time: number, x: number, y: number, active: number, travel: number, ripples: Ripple[]) {
        ...
        gl.uniform4fv(uniforms['rings[0]']!,new Float32Array(Array.from({length:RIPPLE_LIMIT},(_,i) => {
          const r=ripples[i]; return r ? [r.x,1-r.y,time-r.born,r.strength] : [0,0,-100,0]
        }).flat()))
        gl.drawArrays(gl.TRIANGLES,0,3)
      },
      destroy,
    }
  } catch { destroy(); return undefined }
}

/** Passive observers add decoration without intercepting application controls. */
export function createInteractiveFlow(layer: HTMLElement, canvas: HTMLCanvasElement, theme: Theme) {
  const context = canvas.getContext('2d')
  const gpu = document.createElement('canvas')
  gpu.setAttribute('aria-hidden','true'); layer.prepend(gpu)
  let shader = fluid(gpu,theme)
  layer.dataset.flowRenderer = shader ? 'webgl' : 'canvas'
  layer.dataset.flowScene = theme.scene
  if (!shader) gpu.hidden=true
  ...
  const addRipple=(x:number,y:number,strength:number) => {
    ripples.push({x,y,born:time,strength});if(ripples.length>RIPPLE_LIMIT)ripples.shift()
  }
  ...
  const move=(event:PointerEvent) => {
    ...
    if(scene!=='flow' && pointer.inside && time-trail.born>=.14 && Math.hypot((x-trail.x)*width,(y-trail.y)*height)>18){
      addRipple(x,y,.34);trail.x=x;trail.y=y;trail.born=time
    }
  }
  ...
  const click=(event:PointerEvent) => {
    ...
    pointer.targetX=x;pointer.targetY=y;pointer.inside=true
    addRipple(x,y,1)
  }
  const onScroll=(event:Event) => {
    if(!enabled) return
    const target=event.target
    if(target instanceof Element && (target.closest('[aria-label="QuantSkills 插件应用"]') || target.hasAttribute('data-conversation-scroll'))) scroll=target.scrollTop
  }
  let repaint = () => {}
  const lost=(event:Event)=>{ event.preventDefault(); shader?.destroy();shader=undefined;gpu.hidden=true;layer.dataset.flowRenderer='canvas';repaint() }
  gpu.addEventListener('webglcontextlost',lost)
  document.addEventListener('pointermove',move,{passive:true});document.addEventListener('pointerdown',click,{passive:true})
  document.documentElement.addEventListener('pointerleave',leave);document.addEventListener('scroll',onScroll,{capture:true,passive:true})
  ...
  const wash=(x:number,y:number,radius:number,color:string,alpha:number) => {
    ...
    gradient.addColorStop(0,color);gradient.addColorStop(.45,color);gradient.addColorStop(1,'transparent')
    context.globalAlpha=alpha;context.fillStyle=gradient;context.fillRect(x-radius,y-radius,radius*2,radius*2)
  }
  const fallback=() => {
    if(!context)return
    context.globalAlpha=1;context.fillStyle=theme.tokens.deep;context.fillRect(0,0,width,height)
    if(scene==='ink'){
      for(let j=0;j<3;j++){
      ...
      for(let j=0;j<3;j++){
        context.globalAlpha=.085+j*.025;context.fillStyle=theme.tokens.ink;context.beginPath();context.moveTo(-20,height)
        ...
        context.lineTo(width+40,height);context.closePath();context.fill()
        for(let cloud=0;cloud<3;cloud++)wash(width*(.15+cloud*.4+Math.sin(time*.025+j)*.04),height*(.64+j*.085),width*.25,theme.tokens.deep,.075)
      }
    }else{
      for(let j=0;j<4;j++){
  ...
  const rain=() => {
    ...
    for(let i=0;i<128;i++){
      ...
      context.globalAlpha=(front?.19:.10)*(1-clear*.88);context.lineWidth=front?1:.65
      context.beginPath();context.moveTo(x,y);context.lineTo(x-3-(front?3:0),y+12+(front?19:0));context.stroke()
    }
    for(let i=0;i<18;i++){
      ...
      context.strokeStyle=theme.tokens.ink;context.globalAlpha=.18*(1-clear*.9);context.lineWidth=.7
      context.beginPath();context.ellipse(x,y,size,size*(1.2+i%3*.4),-.16,0,Math.PI*2);context.stroke()
      context.fillStyle=theme.tokens.ink;context.globalAlpha*=.75;context.beginPath();context.arc(x-size*.35,y-size*.5,.55,0,Math.PI*2);context.fill()
    }
  }
  const renderer = {
    setActive(active: boolean) {
      enabled=active
      if(!active){pointer.inside=false;pointer.strength=0;ripples.length=0;trail.born=-1}
    },
    resize(w:number,h:number) { width=Math.max(1,w);height=Math.max(1,h);shader?.resize(width,height) },
    draw(now:number) {
      if(disposed)return
      if(enabled || !painted)time=now
      painted=true
      if(enabled){
      ...
      while(ripples[0] && time-ripples[0].born>4)ripples.shift()
      ...
      for(let i=0;i<(scene==='flow'?100:scene==='ink'?38:58);i++){
        ...
        x+=(dx*.12-dy*.18)*force;y+=(dy*.12+dx*.18)*force
        context.fillStyle=colors[i%colors.length]!
        context.globalAlpha=(light?.10:.12)+(light?.19:.38)*(.5+.5*Math.sin(time*p.speed+i))+force*.25
        context.beginPath();context.arc(x,y,p.size*(scene==='ink'?.75:1)+force*.7,0,Math.PI*2);context.fill()
        if(scene==='flow' && force>.1)near.push({x,y})
        if(scene==='flow' && i%23===0){context.globalAlpha*=.55;context.fillRect(x-3,y-.4,6,.8);context.fillRect(x-.4,y-3,.8,6)}
      }
      context.lineWidth=.6;context.strokeStyle=theme.tokens.agent
      ...
      for(const r of ripples){
        ...
        if(scene==='ink')wash(r.x*width,r.y*height,28+age*55,theme.tokens.ink,Math.exp(-age)*.15*r.strength)
        context.strokeStyle=theme.tokens.accent;context.lineWidth=scene==='ink'?1:1.1
        context.globalAlpha=Math.exp(-age*1.2)*(scene==='ink'?.52:light?.36:.55)*r.strength
        context.beginPath();context.arc(r.x*width,r.y*height,radius,0,Math.PI*2);context.stroke()
        context.strokeStyle=scene==='lagoon'?theme.tokens.agent:theme.tokens.team;context.globalAlpha*=.45
        context.beginPath();context.arc(r.x*width,r.y*height,radius*.82,0,Math.PI*2);context.stroke()
      }
      ...
      if(scene==='flow' && phase<2.4){
        ...
        trail.addColorStop(0,'transparent');trail.addColorStop(.7,theme.tokens.agent);trail.addColorStop(1,theme.tokens.ink)
        context.globalAlpha=Math.sin(phase/2.4*Math.PI)*.65;context.strokeStyle=trail;context.lineWidth=1.4
        context.beginPath();context.moveTo(x-100,y-30);context.lineTo(x,y);context.stroke()
      }
      context.globalAlpha=1
    },
    destroy(){
      if(disposed)return
      disposed=true;enabled=false;ripples.length=0
      shader?.destroy();gpu.remove();gpu.removeEventListener('webglcontextlost',lost)
      document.removeEventListener('pointermove',move);document.removeEventListener('pointerdown',click)
      document.documentElement.removeEventListener('pointerleave',leave);document.removeEventListener('scroll',onScroll,true)
    },
  }
