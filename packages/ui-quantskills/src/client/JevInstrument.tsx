...
export function JevInstrument({ config, onChange, catalog = products, compact = false }: { config: ContestWatchConfig; onChange: (value: ContestWatchConfig) => void; catalog?: readonly FuturesProduct[]; compact?: boolean }) {
  ...
  return <>
    ...
    {compact ? <details className="qs-instrument-parameters" open={!instrument.tickSize || !known}><summary>合约参数 · {instrument.exchange} · tick {instrument.tickSize || '待填写'}</summary>{parameters}</details> : parameters}
  </>
}
