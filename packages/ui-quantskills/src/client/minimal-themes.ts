...
export const findMinimalTheme = (background: string) => MINIMAL_THEMES.find(theme => theme.background === background)
export function minimalThemeStyle(theme: typeof MINIMAL_THEMES[number]): Record<string, string> {
  return Object.fromEntries([
    ...Object.entries(theme.tokens).map(([key, value]) => [`--qs-${key}`, value]),
    ['--qs-flow-rgb', theme.glow],
  ])
}
export function minimalBackgroundImage(theme: typeof MINIMAL_THEMES[number]) {
