...
export function JevConnection({ access, models }: {
  ...
  useEffect(() => {
    ...
    void waitForCompetition(() => access.settings(), 'Jev 配置').then(value => {
      if (!disposed) { setSettings(value); setTranslation(value.translator ? JSON.stringify(value.translator) : '') }
  ...
  return <details className={css.jevConnection} open={settings ? !settings.configured : true}>
    ...
    <div className={css.jevConnectionBody}>
      ...
      <form className={css.jevKeyForm} onSubmit={event => { event.preventDefault(); void test(true) }}>
        ...
        <button type="submit" data-primary disabled={busy || !settings?.writable || !key.trim()}>{busy ? '正在测试连接…' : '测试并保存密钥'}</button>
        <button type="button" disabled={busy || !settings?.configured} onClick={() => { void test(false) }}>测试已保存连接</button>
      </form>
      <div className={css.jevKeyForm}>
        <label>中文专用翻译模型<select value={translation} disabled={busy} onChange={event => setTranslation(event.target.value)}>
        ...
        <button type="button" disabled={busy || !translation} onClick={async () => {
          setBusy(true); setError('')
