 ...
 */
export function buildSkillGuidePrompt(
  conversation: string,
  skills: readonly QuantSkillsRecommendableSkill[],
): string {
...
function parseModelObject(answer: string): Record<string, unknown> {
  const start = answer.indexOf('{')
  const end = answer.lastIndexOf('}')
  if (start < 0 || end < start) throw new Error('AI 技能 导购返回了无效结果。')
  let value: unknown
  try {
...
function requireGuideText(value: unknown, field: string, maximum: number): string {
  if (typeof value !== 'string') throw new Error(`AI 技能 导购结果缺少 ${field}。`)
  const text = value.replace(/\s+/gu, ' ').trim()
  if (text === '' || text.length > maximum) throw new Error(`AI 技能 导购结果的 ${field} 无效。`)
  return text
}
 ...
 */
export function parseSkillGuideDecision<T extends QuantSkillsRecommendableSkill>(
  answer: string,
  skills: readonly T[],
): QuantSkillsGuideDecision<T> {
  ...
  if (kind === 'clarify') {
    ...
    if (new Set(options).size !== options.length) throw new Error('AI 技能 导购返回了重复选项。')
  ...
  if (kind === 'recommend') {
    ...
    const recommendations = value.recommendations.map((candidate) => {
      ...
      const asset = (candidate as { readonly asset?: unknown }).asset
      if (typeof asset !== 'string') throw new Error('AI 技能 导购候选缺少 asset。')
      if (seen.has(asset)) throw new Error(`AI 技能 导购重复推荐了「${asset}」。`)
      seen.add(asset)
      const skill = skills.find(entry => entry.name === asset)
      if (skill === undefined) throw new Error(`AI 技能 导购返回了目录外的项目「${asset}」。`)
      const reason = requireGuideText((candidate as { readonly reason?: unknown }).reason, 'reason', 240)
      return Object.freeze({ skill, reason })
    })
    return Object.freeze({ kind, message, recommendations: Object.freeze(recommendations) })
  }
  if (kind === 'create') return Object.freeze({ kind, message })
