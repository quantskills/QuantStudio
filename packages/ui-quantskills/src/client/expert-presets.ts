import type { QuantSkillsAgentCreateRequest, QuantSkillsAgentDefinition, QuantSkillsInstalledVersionId } from './plugin-types.ts'
import type { QuantSkillsInstalledVersion } from './types.ts'
import { OFFICE_EXPERT_PRESETS, OFFICE_DELIVERY_RULES } from './office-presets.ts'

export const PRESET_GROUPS = { investing: '投资研究', quant: '量化研究', office: '日常办公' } as const
...
export const EXPERT_PRESETS: readonly ExpertPreset[] = [
...
export const expertMarker = (preset: ExpertPreset) => `<!-- quantskills-expert:${preset.id}:v1 -->`
export const findPresetExpert = (preset: ExpertPreset, definitions: readonly QuantSkillsAgentDefinition[]) => definitions.find(item => item.role.includes(expertMarker(preset)))

export function expertRole(preset: ExpertPreset): string {
...
export function presetRequest(preset: ExpertPreset, versions: readonly QuantSkillsInstalledVersion[]): QuantSkillsAgentCreateRequest {
  const versionIds = preset.skills.flatMap(name => {
    ...
    return version ? [version.versionId as QuantSkillsInstalledVersionId] : []
  })
