...
import { products, type FuturesProduct } from './jev-products.ts'
...
export function configuredInstrument(config: ContestWatchConfig, catalog: readonly FuturesProduct[] = products): Instrument {
...
export function withInstrument(config: ContestWatchConfig, instrument: Instrument): ContestWatchConfig {
...
export function instrumentIssue(config: ContestWatchConfig, catalog: readonly FuturesProduct[] = products): string | undefined {
  ...
  if (instrument.product.toLowerCase() !== product) return '合约与所选品种不一致，请重新选择品种或填写实际合约。'
  if (known && known.exchange !== instrument.exchange) return `该品种属于${exchanges[known.exchange]}，请核对交易所。`
  if (!Number.isFinite(instrument.tickSize) || instrument.tickSize <= 0) return '请填写该合约的最小价格变动（tick），必须大于 0。'
  return undefined
}

/** Starting values for a simulation workflow, not an optimized or backtested strategy. */
export function rangeTemplate(symbol = ''): ContestWatchConfig {
