import { EXPERT_PRESETS, findPresetExpert, presetRequest } from './expert-presets.ts'
...
import { OFFICE_TEAM_PRESETS, OFFICE_DELIVERY_RULES } from './office-presets.ts'
...
export interface TeamPreset {
  ...
  output: string
  lead: string
  members: readonly { expert: string; responsibility: string }[]
...
export function teamPresetRequest(preset: TeamPreset, definitions: readonly QuantSkillsAgentDefinition[]): QuantSkillsAgentTeamCreateRequest {
  const resolve = (id: string) => {
    const expert = expertPresetById(id), saved = findPresetExpert(expert, definitions)
    if (!saved) throw new Error(`请先添加${expert.name}`)
    return saved
  }
  const lead = resolve(preset.lead)
  const members = preset.members.map(member => {
    const expert = resolve(member.expert)
    if (expert.permission !== lead.permission) throw new Error(`${expert.name}与负责人权限不同，请先在专家配置中统一权限，再添加团队。`)
    return { name: member.expert, agentId: expert.agentId, agentRevision: expert.revision, context: 'fresh' as const, model: { kind: 'default' as const } }
  })
...
export async function saveTeamPresets(presets: readonly TeamPreset[], access: TeamPresetAccess): Promise<QuantSkillsAgentTeamDefinition[]> {
    ...
    if (!team) {
      for (const id of [preset.lead, ...preset.members.map(member => member.expert)]) {
        ...
        if (!findPresetExpert(expert, definitions)) {
          ...
          definitions.push(created); access.savedExpert(created)
        }
      }
      team = await access.createTeam(teamPresetRequest(preset, definitions))
      teams.push(team); access.savedTeam(team)
    }
    saved.push(team)
  }
  return saved
