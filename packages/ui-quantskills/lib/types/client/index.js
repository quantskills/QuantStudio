...
import { QuantSkillsAgentsController } from "./agents.js";
import { findPresetExpert, presetRequest } from "./expert-presets.js";
import { saveTeamPresets } from "./team-presets.js";
import { QuantSkillsApp, QuantSkillsFrame, QuantSkillsPluginFrame, QuantSkillsPluginLauncher, QuantSkillsRail, QuantSkillsResultAction, QuantSkillsResultPanel, } from "./QuantSkillsApp.js";
import { readArtifactTheme, subscribeArtifactTheme } from "./artifact-theme.js";
import { installQuantSkillsDocumentBrand, QuantSkillsBrandMark, QuantSkillsConversationHeroIdentity, QuantSkillsConversationStatus, } from "./QuantSkillsBrand.js";
import { createQuantSkillsLayoutStore, createQuantSkillsNotificationStore, createQuantSkillsViewStore, } from "./store.js";
import { bindQuantSkillsResultMentionClicks } from "./result-mention.js";
import { buildSkillGuidePrompt, parseSkillGuideDecision, } from "./task-recommendation.js";
...
import { QuantSkillsGravityLaneModelSelect } from "./QuantSkillsGravityLaneModelSelect.js";
import { QuantSkillsAssetDraftCard, } from "./QuantSkillsAssetDraftCard.js";
import { QuantSkillsCapabilityPicker, QuantSkillsCapabilityPickerController, QuantSkillsResidentControl, quantSkillsLauncherSource, } from "./QuantSkillsCapabilityPicker.js";
import { QUANTSKILLS_APPEARANCE_SETTINGS_NAMESPACE, QUANTSKILLS_ASSET_DISPLAY_NAME_OVERRIDES_FIELD, QUANTSKILLS_AUTO_CHECK_CATALOG_FIELD, QUANTSKILLS_AUTO_CHECK_PANDA_FIELD, QUANTSKILLS_COLOR_SCHEME_FIELD, QUANTSKILLS_CONVERSATION_BRIGHTNESS_FIELD, QUANTSKILLS_CONVERSATION_SCALE_FIELD, QUANTSKILLS_CONVERSATION_OVERLAY_OPACITY_FIELD, QUANTSKILLS_DARK_BACKGROUND_FIELD, QUANTSKILLS_INTERFACE_SCALE_FIELD, QUANTSKILLS_LIGHT_BACKGROUND_FIELD, QUANTSKILLS_DEFAULT_AGENT_MODEL_FIELD, QUANTSKILLS_DEFAULT_AGENT_PERMISSION_FIELD, QUANTSKILLS_DEFAULT_AGENT_PROVIDER_FIELD, QUANTSKILLS_DEFAULT_AGENT_REASONING_EFFORT_FIELD, QUANTSKILLS_DEFAULT_WORKSPACE_ID_FIELD, QUANTSKILLS_FAVORITE_ASSET_IDS_FIELD, QUANTSKILLS_RESUME_AFTER_PANDA_LOGIN_FIELD, } from "../appearance-settings.js";
export { createQuantSkillsLayoutStore, createQuantSkillsNotificationStore, createQuantSkillsViewStore, } from "./store.js";
...
function isExactQuantSkillsBinding(actual, expected) {
...
/** Verify that the runtime's Host projection names the exact version returned by create. */
function hasExactQuantSkillsBinding(summary, expected) {
...
function hasExactQuantSkillsAgent(summary, expected) {
...
function hasExactQuantSkillsAgentTeam(summary, expected) {
...
function hasSameAgentSnapshot(actual, expected) {
...
class QuantSkillsLayoutController {
    actions;
    ...
    closeDetails() { this.requireActions().closeDetails(); }
    openResults() { this.requireActions().openResults(); }
    closeResults() { this.requireActions().closeResults(); }
    ...
    syncDetailsReservation() {
        if (this.actions === undefined)
            return;
        this.actions.setDetailsReservation(Math.max(this.detailsClaim?.width ?? 0, ...this.detailsReservations.values()));
    }
    ...
    requireActions() {
        if (this.actions === undefined)
            throw new Error('QuantSkills layout is not mounted.');
        return this.actions;
    }
}
/**
 ...
 */
export function mountQuantSkillsApplication(ctx, options) {
    const lifetime = new AbortController();
    ctx.effect(installQuantSkillsDocumentBrand, 'ui-quantskills: own document and taskbar branding');
    ctx.effect(() => () => { lifetime.abort(); }, 'ui-quantskills: cancel in-flight Session creation on dispose');
    ...
    const unwrapRemote = async (request) => {
        const response = await request;
        if (!response.ok)
            throw new RemoteFailure(response.error.code, response.error.message);
    ...
    const preparedResultArtifacts = (prepared) => prepared.results.map(result => result.status === 'ready'
        ? {
    ...
    const catalog = new QuantSkillsCatalogController({
        ...
        catalog: signal => unwrapRemote(ctx.remote.quantSkills.catalog(signal)),
        list: signal => unwrapRemote(ctx.remote.quantSkills.list(signal)),
        install: (request, signal) => unwrapRemote(ctx.remote.quantSkills.installAsset(request, signal)),
    });
    const readmeCache = new Map();
    const readAssetReadme = async (asset, callerSignal) => {
        ...
        if (installed) {
            const markdown = await unwrapRemote(ctx.remote.quantSkills.manualSkillRead(installed.versionId, callerSignal));
            return { assetId: asset.name, commit: asset.commitSha, path: 'SKILL.md', markdown };
        }
        ...
        const result = await unwrapRemote(ctx.remote.quantSkills.assetReadme({
            assetId: asset.name,
            observedSnapshotId: current.snapshotId,
    ...
    const applyCatalogSyncStatus = (status) => {
    ...
    const boundSessions = new QuantSkillsSessionsController({
        ...
        create: (request, signal) => unwrapRemote(ctx.remote.quantSkillsSessions.create(request, signal)),
    });
    const agents = new QuantSkillsAgentsController({
        ...
        create: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentCreate(request)),
        update: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentUpdate(request)),
        delete: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentDelete(request)),
        start: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentSessionCreate(request)),
        ...
        teamCreate: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentTeamCreate(request)),
        teamUpdate: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentTeamUpdate(request)),
        teamDelete: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentTeamDelete(request)),
        teamStart: (request) => unwrapRemote(ctx.remote.quantSkillsSessions.agentTeamSessionCreate(request)),
    });
    ...
    ctx.effect(() => inputTriggers.registerSource(quantSkillsLauncherSource(capabilityPicker, attachments)), 'ui-quantskills: composer launcher source');
    ...
    ctx.effect(() => {
        const sync = () => { syncPreferences(preferences.getSnapshot()); };
        sync();
    ...
    ctx.effect(() => {
        ...
        const sync = () => {
            ...
            timer = setTimeout(() => {
                ...
                if (!snapshot.writable || !snapshot.value || !document.body.dataset.qsPluginTheme)
                    return;
                const palette = JSON.stringify(readArtifactTheme());
                if (palette === snapshot.value.artifactPalette || palette === pending)
                    return;
                pending = palette;
    ...
    if (nativeLayout !== undefined)
        layout.attach(nativeLayout.actions);
    let sessionOpenGeneration = 0;
    let suppressedPluginSessionId;
    const viewActions = {
        ...view.actions,
        closePlugin: () => {
    ...
    const adoptSession = (id, signal, accept = () => true) => new Promise((resolve, reject) => {
        let dispose = () => { };
        const finish = (result) => {
        ...
        const inspect = () => {
            ...
            if (summary === undefined || !accept(summary))
                return;
    ...
    const waitForAssistantText = (sessionId, session, operation, signal) => new Promise((resolve, reject) => {
        ...
        const inspect = () => {
            ...
            if (assistantSeen)
                finish({ ok: false, error: new Error(`${operation}失败：模型没有返回正文。`) });
        };
    ...
    const openSession = (id, ensureMissing = true) => {
        ...
        if (summary !== undefined) {
            ...
            if (ctx.sessions.binding(id) !== undefined) {
                if (ensureMissing) {
                    void unwrapRemote(ctx.remote.quantSkillsSessions.sessionEnsure({ sessionId: id }, lifetime.signal)).catch((cause) => {
        ...
        void (async () => {
            if (ensureMissing) {
                await unwrapRemote(ctx.remote.quantSkillsSessions.sessionEnsure({ sessionId: id }, lifetime.signal));
            }
            if (generation !== sessionOpenGeneration || lifetime.signal.aborted)
                return;
    ...
    if (options.mode === 'standalone') {
        ...
        if (nativeLayout === undefined)
            throw new Error('QuantSkills native layout is unavailable.');
        ...
        ctx.effect(() => {
            ...
            const sync = () => {
                ...
                if (current !== observedSessionId) {
                    ...
                    if (current !== suppressedPluginSessionId)
                        suppressedPluginSessionId = undefined;
                }
                if (current === undefined || current === suppressedPluginSessionId)
                    return;
                ...
                if (owned && !view.store.getSnapshot().pluginOpen)
                    view.actions.openPluginConversation();
            };
            const disposers = [
    ...
    ctx.slots.inject('conversation.chat.deliverables', () => ctx.slots.register({
        ...
        inject: (sessionId) => ({
            downloadFile: (path) => artifactDownload(sessionId, path),
            openWorkbench: (path) => {
            ...
            previewFile: (path) => unwrapRemote(ctx.remote.quantSkillsSessions.resultPreview({ sessionId, path })),
        }),
    }, FinalDeliverables));
    ...
    const resolveSessionWorkspace = async () => {
        if (options.mode === 'standalone')
            return legacySessionWorkspace();
        const preferredWorkspaceId = view.store.getSnapshot().defaultWorkspaceId;
        let resolved;
        try {
            resolved = await unwrapRemote(ctx.remote.quantSkillsSessions.workspaceResolve({
    ...
    const startPlainSession = async () => {
        ...
        const requestedSessionId = `session-${crypto.randomUUID()}`;
        const created = await unwrapRemote(ctx.remote.quantSkillsSessions.plainSessionCreate({
            ...workspace,
    ...
    const factorContestAccess = {
        ...
        requestResearch: async (sessionId, text) => {
            const current = ctx.sessions.list.getSnapshot();
            if (current.current !== sessionId || current.byId[sessionId]?.projectionValues?.quantSkillsPlainSession?.purpose !== 'factor-contest')
                throw new Error('请在对应因子比赛会话中发起研究。');
            const session = ctx.sessions.binding(sessionId)?.session;
            if (!session)
                throw new Error('因子会话尚未就绪。');
            const result = await session.prompt([{ type: 'text', text }], 'queue');
            if (!result.ok)
                throw new Error(result.error.message);
        },
    ...
    ctx.slots.inject('conversation.input.left', () => ctx.slots.register({
        ...
        inject: () => ({
            ...
            upload: (sessionId, file, data) => unwrapRemote(ctx.remote.quantSkillsSessions.fileAttach({
                sessionId,
                data,
            ...
            appendDraft: (sessionId, text) => {
                if (conversation === undefined)
                    throw new Error('QuantSkills：会话输入器尚未就绪。');
                const binding = ctx.sessions.binding(sessionId);
                if (binding === undefined)
                    throw new Error('QuantSkills：附件所属会话不可用。');
    ...
    const createBoundSession = async (versionId, initialTask) => {
        const workspace = await resolveSessionWorkspace();
        ...
        const adopted = await adoptSession(created.sessionId, lifetime.signal, summary => hasExactQuantSkillsBinding(summary, created.binding));
        ...
        if (initialTask !== undefined) {
            ...
            if (!response.ok)
                throw new Error(`QuantSkills 任务启动失败：${response.error.message}`);
        }
        viewActions.navigate('conversations');
        return adopted.sessionId;
    };
    const startSkillSession = async (_asset, version) => {
        await createBoundSession(version.versionId);
    };
    const startBoundSession = async (binding, _label) => {
        await createBoundSession(binding.versionId);
    };
    const startAgentSessionCore = async (definition, initialTask, authoringKind) => {
        const workspace = await resolveSessionWorkspace();
        const sessionId = `session-${crypto.randomUUID()}`;
        const request = {
            sessionId,
        ...
        const created = authoringKind === undefined
            ...
            : await unwrapRemote(ctx.remote.quantSkillsSessions.authoringSessionCreate({ ...request, kind: authoringKind }));
        const adopted = await adoptSession(created.sessionId, lifetime.signal, summary => hasExactQuantSkillsAgent(summary, created.agent));
        lifetime.signal.throwIfAborted();
        ...
        const permission = await ctx.remote.commands.execute(adopted.sessionId, `/permission ${created.agent.permission}`, []);
        if (!permission.ok)
            throw new Error(`QuantSkills 专家 权限设置失败：${permission.error.message}`);
        if (permission.value === undefined)
            throw new Error('QuantSkills 专家 权限命令不可用。');
        openSession(adopted.sessionId);
        if (initialTask !== undefined) {
            ...
            if (!response.ok)
                throw new Error(`QuantSkills 专家 测试失败：${response.error.message}`);
        }
        viewActions.navigate('conversations');
        return adopted.sessionId;
    };
    const startAgentSession = (definition, initialTask) => {
        return startAgentSessionCore(definition, initialTask);
    };
    const startAgentTeamSessionCore = async (definition, initialTask) => {
        const workspace = await resolveSessionWorkspace();
        const sessionId = `session-${crypto.randomUUID()}`;
        ...
        const adopted = await adoptSession(created.sessionId, lifetime.signal, summary => hasExactQuantSkillsAgentTeam(summary, created.team));
        lifetime.signal.throwIfAborted();
        ...
        const permission = await ctx.remote.commands.execute(adopted.sessionId, `/permission ${lead.permission}`, []);
        if (!permission.ok)
            throw new Error(`QuantSkills 专家团 权限设置失败：${permission.error.message}`);
        if (permission.value === undefined)
            throw new Error('QuantSkills 专家团 权限命令不可用。');
        openSession(adopted.sessionId);
        if (initialTask !== undefined) {
            const response = await adopted.session.prompt([{ type: 'text', text: initialTask }], 'queue');
            if (!response.ok)
                throw new Error(`QuantSkills 专家团 启动失败：${response.error.message}`);
        }
        viewActions.navigate('conversations');
        return adopted.sessionId;
    };
    const startAgentTeamSession = (definition, initialTask) => {
        return startAgentTeamSessionCore(definition, initialTask);
    };
    const startAuthoringSession = async (kind, initialRequest) => {
        ...
        const sessionId = await startAgentSessionCore(definition, quantSkillsAuthoringOpening(kind, initialRequest), kind);
    ...
    const listAgentModels = async () => {
        const response = await ctx.remote.session.modelCatalog();
        if (!response.ok)
            throw new Error(`QuantSkills 模型目录读取失败：${response.error.message}`);
    ...
    const generateAgentRole = async (request, callerSignal) => {
        const signal = callerSignal === undefined
            ? lifetime.signal
            : AbortSignal.any([lifetime.signal, callerSignal]);
        ...
        try {
            ...
            if (request.model !== undefined) {
                ...
                if (!selected.ok)
                    throw new Error(`AI 角色说明模型选择失败：${selected.error.message}`);
            }
            ...
            if (!response.ok)
                throw new Error(`AI 角色说明生成失败：${response.error.message}`);
            return await waitForAssistantText(sessionId, session, 'AI 角色说明生成', signal);
        }
        finally {
            if (adopted?.session.getSnapshot().running)
                await adopted.session.cancel();
            await ctx.workspaces.archiveSession(sessionId).catch((error) => {
                console.warn('QuantSkills AI role helper could not archive its temporary Session.', error);
            });
            if (current === undefined)
                ctx.sessions.clear();
            else if (ctx.sessions.list.getSnapshot().byId[current] !== undefined)
                ctx.sessions.open(current);
        }
    };
    const matchSkillForTask = async (request, skills) => {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`AI 技能 匹配失败：${response.error.message}`);
    ...
    const importAgent = async (asset, version) => {
        const template = await unwrapRemote(ctx.remote.quantSkills.agentTemplate(version.versionId));
        const snapshot = catalog.source.getSnapshot();
        const requiredVersions = template.requires.map((assetId) => {
            ...
            if (installed === undefined)
                throw new Error(`专家 依赖 ${assetId} 尚未安装。`);
    ...
    const installAgent = async (asset) => {
    ...
    const detachResidentSkill = async (sessionId, assetId) => {
        await unwrapRemote(ctx.remote.quantSkillsSessions.residentSkillDetach({
            sessionId,
            assetId: assetId,
        }));
    };
    const toggleResidentSkill = async (sessionId, asset, attached) => {
        ...
        await unwrapRemote(ctx.remote.quantSkillsSessions.residentSkillAttach({
            sessionId,
            versionId: version.versionId,
        }));
    };
    const openCatalogAgent = async (asset) => {
    ...
    ctx.slots.inject('conversation.input.overlay', () => ctx.slots.register({
        ...
        inject: () => ({
            ...
            openTeamPreset: async (preset) => {
                ...
                if (team)
                    await startAgentTeamSession(team);
            },
        }),
    ...
    ctx.slots.inject('conversation.input.left', () => ctx.slots.register({
        ...
        inject: () => ({
            ...
            listPromptForms: sessionId => unwrapRemote(ctx.remote.quantSkillsSessions.promptFormList({ sessionId })),
            renderPromptForm: request => unwrapRemote(ctx.remote.quantSkillsSessions.promptFormRender(request)).then(result => result.text),
            fillDraft: (sessionId, text) => {
                if (conversation === undefined)
                    throw new Error('QuantSkills：会话输入器尚未就绪。');
                const binding = ctx.sessions.binding(sessionId);
                if (binding === undefined)
                    throw new Error('QuantSkills：参数所属会话不可用。');
    ...
    ctx.slots.inject('conversation.input.left', () => ctx.slots.register({
        ...
        inject: () => ({ ...conversationBrand(), ...pandaMcp }),
    ...
    const send = async (text) => {
        ...
        const session = ctx.sessions.binding(sessionId)?.session;
        if (session === undefined)
            throw new Error('QuantSkills：当前会话不可用。');
        const result = await session.prompt([{ type: 'text', text }], 'queue');
        if (!result.ok)
            throw new Error(`QuantSkills 提示失败：${result.error.code}: ${result.error.message}`);
    };
    const startTask = async (text, assetName) => {
        if (catalog.source.getSnapshot().phase !== 'ready')
        ...
        await createBoundSession(version.versionId, text);
    ...
    if (options.mode === 'standalone') {
        ...
        }, QuantSkillsRail), 'ui-quantskills: standalone navigation rail');
    }
    ...
    ctx.slots.inject('conversation.session.header.actions', () => ctx.slots.register({
        ...
        inject: () => ({
            ...
            commit: async (sessionId, toolCallId, digest) => {
                const result = await unwrapRemote(ctx.remote.quantSkillsSessions.authoringCommit({ sessionId, toolCallId, expectedTreeDigest: digest }));
                await Promise.all([catalog.refreshInstalled(), agents.refresh()]);
                return result;
            },
            start: async (result) => {
                if (result.kind === 'skill')
                    await createBoundSession(result.version.versionId);
                else if (result.kind === 'agent')
                    await startAgentSession(result.agent);
                else
                    await startAgentTeamSession(result.team);
            },
            openLibrary: kind => {
                if (kind === 'skill') {
                    try {
                        localStorage.setItem('quantskills.skillLibrarySource', 'mine');
                    }
                    catch { /* Optional preference. */ }
                }
                view.actions.navigate(kind === 'skill' ? 'skills' : kind === 'agent-team' ? 'teams' : 'agents');
                if (kind !== 'skill')
                    view.actions.setAgentWorkspaceTab(kind === 'agent-team' ? 'teams' : 'mine');
            },
    ...
    ctx.slots.inject('conversation.input.dock', () => ctx.slots.register({
        ...
        inject: () => ({ hooks: { sessions: ctx.sessions.list }, access: factorContestAccess,
    ...
    ctx.slots.inject('quantskills.results', () => ctx.slots.register({
        ...
        inject: (sessionId) => ({
            ...
            listFiles: async (signal) => preparedResultArtifacts(await unwrapRemote(ctx.remote.quantSkillsSessions.resultList({ sessionId }, signal))),
            prepareFiles: async (paths, signal) => {
                const prepared = await unwrapRemote(ctx.remote.quantSkillsSessions.resultPrepare({ sessionId, paths }, signal));
            ...
            saveFileAs: async (path) => { artifactDownload(sessionId, path); },
            previewFile: path => unwrapRemote(ctx.remote.quantSkillsSessions.resultPreview({ sessionId, path })).catch((error) => {
            ...
            continueWithResults: async (paths) => {
                ...
                if (skill !== undefined) {
                    ...
                    if (definition === undefined)
                        throw new Error('QuantSkills：当前 专家 定义不可用于创建后续会话。');
                    ...
                    if (definition === undefined)
                        throw new Error('QuantSkills：当前 专家团 定义不可用于创建后续会话。');
                ...
                if (next === undefined || conversation === undefined)
                    throw new Error('QuantSkills：后续会话尚未就绪。');
    ...
    ctx.slots.inject('tool.call.toolview', () => ctx.slots.register({
        ...
        inject: (sessionId) => ({
            ...
            commitAuthoring: async (toolCallId, treeDigest) => {
                const result = await unwrapRemote(ctx.remote.quantSkillsSessions.authoringCommit({
                    sessionId,
                    toolCallId,
            ...
            openAgentTeamBuilder: (seed) => { view.actions.requestAgentTeamCreation(seed); },
            focusComposer: () => {
                document.querySelector('[data-composer-card] textarea:not(:disabled)')?.focus();
            },
        }),
    ...
    ctx.slots.inject('quantskills.page', () => ctx.slots.register({
        ...
        inject: () => ({
            ...
            applicationUpdateCheck: (request) => unwrapRemote(ctx.remote.quantSkills.applicationUpdateCheck(request)),
            ...
            renderPluginMarket: () => {
                ...
                if (!entry || entry.children || entry.store)
                    return createElement('p', { role: 'status' }, '插件市场尚未就绪，请稍后重新打开设置。');
                return createElement(entry.component, { close: () => view.actions.navigate('home') });
            },
            databaseAccess: {
            ...
            uninstallAgent: async (request) => {
                try {
                    await unwrapRemote(ctx.remote.quantSkillsSessions.agentUninstall(request));
                }
                finally {
                    await Promise.all([agents.refresh(), catalog.refreshInstalled()]);
                }
            },
            openSession,
            startSession: () => startPlainSession(),
            startSkillSession,
            startBoundSession,
            readSkillDeclaration: (versionId, signal) => unwrapRemote(ctx.remote.quantSkills.manualSkillRead(versionId, signal)),
            manualSkillSave: async (request) => {
                const saved = await unwrapRemote(ctx.remote.quantSkills.manualSkillSave(request));
                await catalog.refresh();
                return saved;
            },
            ...
            testAgent: definition => startAgentSession(definition, '这是一个隔离测试会话。请验证角色、权限、模型和 技能 编排是否加载成功；只汇报检查结果，不执行真实交易或外部副作用。'),
            listAgentModels,
            generateAgentRole,
            installAgent,
            ...
            workspaceStatus: () => {
                const preferredWorkspaceId = view.store.getSnapshot().defaultWorkspaceId;
                return unwrapRemote(ctx.remote.quantSkillsSessions.workspaceStatus({
                    ...(preferredWorkspaceId === undefined ? {} : { preferredWorkspaceId }),
                }));
            },
            setDefaultWorkspace: async (workspaceId) => {
                if (workspaceId !== undefined) {
                    ...
                    if (!available)
                        throw new Error('所选工作区已不可用。');
                }
                if (workspaceId === undefined)
                    await preferences.unset(QUANTSKILLS_DEFAULT_WORKSPACE_ID_FIELD);
                else
                    await preferences.set(QUANTSKILLS_DEFAULT_WORKSPACE_ID_FIELD, workspaceId);
    ...
    ctx.effect(() => {
        const currentSessionIds = () => Object.keys(ctx.sessions.list.getSnapshot().byId)
            .sort()
            .join('\u0000');
        let observedSessionIds = currentSessionIds();
        const refresh = () => {
            const nextSessionIds = currentSessionIds();
            if (nextSessionIds === observedSessionIds)
                return;
...
export function apply(ctx) {
    const mode = ctx.slots.spec('shell.overlay') === undefined
        ? 'standalone'
        : 'native-plugin';
 ...
 */
export function quantSkillsAuthoringAgentName(kind) {
 ...
 */
export function quantSkillsAuthoringRole(kind) {
    if (kind === 'agent-team') {
        return [
 ...
 */
export function quantSkillsAuthoringOpening(kind, initialRequest) {
 ...
 */
export function quantSkillsAssetRequiresPanda(asset, assets) {
    const byId = new Map(assets.map(candidate => [candidate.name, candidate]));
    const visited = new Set();
    const visit = (candidate) => {
        if (candidate.name === PANDA_CAPABILITY_ASSET_ID)
            return true;
        if (visited.has(candidate.name))
            return false;
        visited.add(candidate.name);
        return candidate.requires.some((requirement) => {
            if (requirement === PANDA_CAPABILITY_ASSET_ID)
                return true;
