...
function useQuantSkillsConversation(mode, useView) {
 ...
 */
export function installQuantSkillsDocumentBrand() {
    const displaced = [...document.head.querySelectorAll('link[rel~="icon"], link[rel="manifest"], link[rel="apple-touch-icon"]')];
 ...
 */
export function QuantSkillsBrandMark({ size, className }) {
    return _jsx("img", { src: markUrl, className: `${css.brandAsset} ${css.brandMark}${className === undefined ? '' : ` ${className}`}`, style: { width: size, height: size }, alt: "", "aria-hidden": "true" });
}
/**
 ...
 */
export function QuantSkillsConversationHeroIdentity({ mode, useView, defaultMark, defaultHeadline, size, className, renderIdentity, }) {
    if (!useQuantSkillsConversation(mode, useView)) {
        return _jsx(_Fragment, { children: renderIdentity(defaultMark, defaultHeadline) });
    }
    return _jsx(_Fragment, { children: renderIdentity(_jsx(QuantSkillsBrandMark, { size: size, className: className }), 'QuantSkills 量化研究工作台') });
}
/**
 ...
 */
export function QuantSkillsConversationStatus({ mode, useView, defaultLabel, }) {
    if (!useQuantSkillsConversation(mode, useView))
        return _jsx(_Fragment, { children: defaultLabel });
    return _jsx(QuantSkillsResearchStatus, {});
}
...
function QuantSkillsResearchStatus() {
    ...
    return _jsxs("span", { className: css.researchStatus, role: "status", "aria-label": "QuantSkills \u6B63\u5728\u5206\u6790\uFF0C\u8BF7\u7A0D\u5019", children: [_jsxs("span", { className: css.researchStatusMark, "aria-hidden": "true", children: [_jsx("img", { src: researchMarkLight, className: css.researchStatusMarkLight, alt: "" }), _jsx("img", { src: researchMarkDark, className: css.researchStatusMarkDark, alt: "" }), _jsx("img", { src: markUrl, className: css.researchStatusStatic, alt: "" })] }), _jsx("span", { className: css.researchPhrase, "aria-hidden": "true", children: RESEARCH_PHRASES[phrase] }, phrase)] });
}
/**
...
export function QuantSkillsBrandLockup() {
    return _jsxs("span", { className: css.brandEndorsedLockup, role: "img", "aria-label": "QuantSkills by PandaAI", children: [_jsx("img", { src: lockupUrl, className: `${css.brandAsset} ${css.brandLockup}`, alt: "", "aria-hidden": "true" }), _jsx("i", { className: css.brandDivider, "aria-hidden": "true" }), _jsxs("span", { className: css.brandEndorsement, "aria-hidden": "true", children: [_jsx("small", { children: "by" }), _jsx("img", { src: pandaMarkUrl, className: `${css.brandAsset} ${css.pandaBrandMark}`, alt: "" }), _jsx("b", { children: "PandaAI" })] })] });
}
//# sourceMappingURL=QuantSkillsBrand.js.map