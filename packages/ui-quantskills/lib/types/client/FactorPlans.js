...
export function FactorPlans({ status, access, refresh, compact = false }) {
    ...
    const controller = useRef(undefined), submitted = useRef(new Set());
    ...
    const perform = async (work, submission) => {
        ...
        try {
            ...
            if (current === generation.current)
                setError(`${e instanceof Error ? e.message : '操作未完成。'}${submission ? '本次确认结果待核实，请勿重复提交。' : ''}`);
        }
        finally {
            if (current === generation.current) {
                active.current = false;
