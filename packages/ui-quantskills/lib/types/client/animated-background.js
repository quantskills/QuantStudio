import { createInteractiveFlow } from "./interactive-flow.js";
import { useEffect, useRef, useSyncExternalStore } from 'react';
import { findMinimalTheme } from "./minimal-themes.js";
...
export function useAnimatedBackground(enabled, background, inConversation = false) {
    const playing = useBackgroundMotion();
    const playingRef = useRef(playing);
    const syncRef = useRef(undefined);
    playingRef.current = playing;
    useEffect(() => {
        if (!enabled || !background.id.startsWith('motion-') || (!background.url && !background.gradient))
            return;
        ...
        const paint = (time) => {
            ...
            for (const p of particles) {
                ...
                if (rain) {
                    context.moveTo(x, y);
                    context.lineTo(x - 3, y + 10 + p.size * 5);
                    context.stroke();
                }
                else {
            ...
            if (scene === 'ocean' || scene === 'meadow' || scene === 'jiangnan') {
                ...
                glow.addColorStop(0, scene === 'meadow' ? '#ffedb119' : '#d4faff12');
        ...
        const draw = (now) => {
            ...
            if (!context || now - previous < 33)
                return;
            const delta = Math.min(50, now - (previous || now));
            previous = now;
            elapsed += delta;
            paint(elapsed / 1000);
        };
        ...
        resize();
        sync();
        const onResize = () => { resize(); if (layer.dataset.playing !== 'true')
            paint(elapsed / 1000); };
