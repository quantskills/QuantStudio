...
export function ContestPage({ access, researchSessions = [], openResearch }) {
    if (!access)
        return _jsxs("section", { className: css.page, children: [_jsx("h1", { children: "\u671F\u8D27\u4EFF\u771F\u6BD4\u8D5B" }), _jsx("p", { children: "\u6BD4\u8D5B\u529F\u80FD\u6682\u672A\u5C31\u7EEA\uFF0C\u8BF7\u91CD\u65B0\u542F\u52A8\u5E94\u7528\u3002" })] });
...
function ConnectedContestPage({ access, researchSessions = [], openResearch }) {
    ...
    useEffect(() => {
        ...
        setDataError(undefined);
        setLoading(false);
        if (connected && tab !== 'quote')
            void query();
        return () => { request.current++; };
    }, [tab, date, board, connected, status?.identity?.accountId, status?.identity?.contestId]);
    useEffect(() => {
        if (!status?.enabled || !status.cliVersion || ['installing', 'authenticating'].includes(status.phase)) {
