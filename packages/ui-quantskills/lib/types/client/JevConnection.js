...
export function JevConnection({ access, models }) {
    ...
    return _jsxs("details", { className: css.jevConnection, open: settings ? !settings.configured : true, children: [_jsxs("summary", { children: [_jsx("span", { children: "Jev \u00B7 TypeSafe" }), _jsx("span", { className: css.jevBadge, "data-ready": settings?.configured, children: settings?.configured ? '密钥已配置' : '等待配置' })] }), _jsxs("div", { className: css.jevConnectionBody, children: [_jsxs("p", { children: ["\u5728 ", _jsx("a", { href: "https://docs.typesafe.ai/introduction", target: "_blank", rel: "noreferrer", children: "TypeSafe" }), " \u83B7\u53D6\u81EA\u5DF1\u7684 API Key\uFF0C\u6D4B\u8BD5\u6210\u529F\u540E\u4FDD\u5B58\u3002\u6BCF\u4F4D\u7528\u6237\u5728\u81EA\u5DF1\u7684\u7535\u8111\u4E0A\u5B8C\u6210\u914D\u7F6E\u3002"] }), _jsx("p", { children: "\u6BD4\u8D5B\u9875\u7684 Jev \u76EF\u76D8\u4E0E\u679C\u8747\u7684 Jev \u8F85\u52A9\u5171\u7528\u6B64\u5BC6\u94A5\uFF0C\u4FEE\u6539\u540E\u4E24\u5904\u540C\u65F6\u751F\u6548\u3002\u76EF\u76D8\u8FD0\u884C\u65F6\uFF0C\u8BF7\u5148\u505C\u6B62\u518D\u4FEE\u6539\u8FDE\u63A5\u3002" }), _jsxs("form", { className: css.jevKeyForm, onSubmit: event => { event.preventDefault(); void test(true); }, children: [_jsxs("label", { children: ["Jev API Key", _jsx("input", { type: "password", autoComplete: "new-password", spellCheck: false, required: true, maxLength: 4096, value: key, disabled: busy || settings?.writable === false, placeholder: settings?.configured ? '输入新密钥以替换，已保存密钥不回显' : '粘贴 TypeSafe API Key', onChange: event => setKey(event.target.value) })] }), _jsx("button", { type: "submit", "data-primary": true, disabled: busy || !settings?.writable || !key.trim(), children: busy ? '正在测试连接…' : '测试并保存密钥' }), _jsx("button", { type: "button", disabled: busy || !settings?.configured, onClick: () => { void test(false); }, children: "\u6D4B\u8BD5\u5DF2\u4FDD\u5B58\u8FDE\u63A5" })] }), _jsxs("div", { className: css.jevKeyForm, children: [_jsxs("label", { children: ["\u4E2D\u6587\u4E13\u7528\u7FFB\u8BD1\u6A21\u578B", _jsxs("select", { value: translation, disabled: busy, onChange: event => setTranslation(event.target.value), children: [_jsx("option", { value: "", children: "\u8BF7\u9009\u62E9\u5DF2\u9A8C\u8BC1\u7684\u6A21\u578B" }), (settings?.translationModels ?? []).map(item => _jsxs("option", { value: JSON.stringify(item), children: [item.model, " \u00B7 ", item.provider] }, JSON.stringify(item)))] })] }), _jsx("button", { type: "button", disabled: busy || !translation, onClick: async () => {
                                    setBusy(true);
                                    setError('');
                                    try {
