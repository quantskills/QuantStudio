...
export const DEFAULT_ARTIFACT_THEME = {
...
const color = (value, fallback) => /^#[0-9a-f]{6}$/i.test(value) ? value
...
export function readArtifactTheme() {
    if (typeof document === 'undefined')
        return DEFAULT_ARTIFACT_THEME;
    const style = getComputedStyle(document.body);
    const dark = document.body.dataset.qsPluginTheme === 'dark';
    const value = (token, fallback) => color(style.getPropertyValue('--qs-' + token).trim(), fallback);
    return {
...
export function subscribeArtifactTheme(notify) {
