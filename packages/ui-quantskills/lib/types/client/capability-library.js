...
export function agentLibrarySource(id, sourceVersionId, records, catalog) {
    const record = records.find(item => item.id === id);
    if (record)
        return record.source === 'personal' ? 'mine' : record.source;
...
export function skillLibraryAssets(catalog, source) {
    if (source === 'discover')
        return catalog.assets.filter(asset => asset.projectType === 'skill');
...
function skillAsset(version, catalogAsset) {
...
//# sourceMappingURL=capability-library.js.map