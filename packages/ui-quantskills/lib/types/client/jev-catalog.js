...
import { futuresExchanges, futuresProductPattern } from '@deepseek-ai/dsh-quantskills-session/contracts';
import { waitForCompetition } from "./competition-async.js";
import { catalogCheckedAt, products } from "./jev-products.js";
/** Official varieties metadata only; never resolves a delivery month or requests market quotes. */
export function parseVarieties(value) {
    const body = value;
    if (!body || !Array.isArray(body.items) || body.items.length === 0 || body.items.length > 1000 || body.total !== body.items.length)
        throw new Error('柜台品种目录不完整，请稍后重新同步。');
    const seen = new Set();
    return body.items.map(value => {
        ...
        if (!item || typeof item.code !== 'string' || !futuresProductPattern.test(item.code) || typeof item.name !== 'string' || !item.name.trim() || item.name.length > 80
            || typeof item.exchange !== 'string' || !Object.hasOwn(futuresExchanges, item.exchange) || typeof item.enabled !== 'boolean')
            throw new Error('柜台品种目录格式不匹配，请检查比赛 CLI 版本。');
        const product = item.code.toLowerCase(), exchange = item.exchange;
        if (seen.has(product))
            throw new Error('柜台品种目录存在重复代码，请核对交易所后手动配置。');
        seen.add(product);
...
export function useJevProducts(access, connected) {
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            ...
            if (version !== generation.current || active.signal.aborted)
                return;
            setCatalog(next);
            setMessage(`已同步柜台目录 · ${next.length} 个品种 · ${next.filter(item => item.enabled).length} 个已启用`);
        }
        catch (error) {
            if (version === generation.current && !active.signal.aborted)
                setMessage(`${error instanceof Error ? error.message : '品种目录同步失败。'} 保留现有目录，仍可手动填写合约。`);
        }
        finally {
            if (version === generation.current && !active.signal.aborted)
                setLoading(false);
    ...
    useEffect(() => {
        setCatalog(products);
        setMessage(`本地目录 · ${catalogCheckedAt} · ${products.length} 个品种`);
        setLoading(false);
        void refresh();
...
//# sourceMappingURL=jev-catalog.js.map