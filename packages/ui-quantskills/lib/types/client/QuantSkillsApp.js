...
export function QuantSkillsPluginFrame({ useView, useLayout, useCatalog, useBoundSessions, useAgents, useSessions, useNotifications, renderSlot, actions, openSession, acknowledgeNotification, renameSession, removeSessions, startSession, startAuthoringSession, openAgentTeamBuilder, claimSidebar, claimDetails, claimConversationTextScale, openResults, closeResults, close, modelAccess, }) {
    ...
    return _jsxs("section", { className: css.pluginFrame, style: {
            ...
            '--qs-background-position': background.position,
                            ...
                            setMobilePanel(undefined); }, children: [conversationOpen && _jsx("span", { className: css.pluginConversationBrand, title: "QuantSkills", children: _jsx(QuantSkillsBrandMark, { size: 30 }) }), NAV_ITEMS.map(item => _jsxs("button", { type: "button", className: clsx(css.pluginNavItem, (page === item.page || (item.page === 'conversations' && page === 'parallel')) && css.pluginNavItemActive), "aria-current": page === item.page ? 'page' : undefined, onClick: () => {
                                ...
                                }, children: [_jsxs("span", { className: css.railIcon, children: [item.icon, item.page === 'conversations' && unreadCount > 0 && _jsx("b", { className: css.badge, children: unreadCount })] }), _jsx("span", { children: item.label })] }, item.page)), _jsxs("div", { className: css.productLinks, role: "group", "aria-label": "PandaAI \u4EA7\u54C1", children: [_jsxs("button", { type: "button", className: clsx(css.pluginNavItem, page === 'qube' && css.pluginNavItemActive), "aria-current": page === 'qube' ? 'page' : undefined, onClick: () => actions.navigate('qube'), title: "QUBE \u00B7 \u4EA7\u54C1\u4ECB\u7ECD", children: [_jsx(Cube, { size: 24, "aria-hidden": "true" }), _jsx("span", { children: "QUBE" })] }), _jsxs("button", { type: "button", className: clsx(css.pluginNavItem, page === 'evo' && css.pluginNavItemActive), "aria-current": page === 'evo' ? 'page' : undefined, onClick: () => actions.navigate('evo'), title: "EVO \u00B7 \u4EA7\u54C1\u4ECB\u7ECD", children: [_jsx(Dna, { size: 24, "aria-hidden": "true" }), _jsx("span", { children: "EVO" })] })] }), _jsxs("button", { type: "button", className: clsx(css.pluginNavItem, page === 'settings' && css.pluginNavItemActive), "aria-current": page === 'settings' ? 'page' : undefined, onClick: () => { actions.navigate('settings'); }, children: [_jsx(GearSix, { size: 24 }), _jsx("span", { children: "\u8BBE\u7F6E" })] })] }), _jsx("div", { ref: mobileSessionsRef, id: mobileSessionsId, className: css.pluginPage, children: _jsx("div", { className: css.pluginScaleViewport, "data-interface-scale-viewport": "page", children: conversationOpen
...
export function QuantSkillsRail({ collapsed, useStore, useNotifications, actions, collapse, renderSlot, }) {
    ...
    const unreadCount = useNotifications(state => Object.keys(state.unreadBySession).length);
    return (_jsxs("nav", { className: css.rail, "aria-label": "QuantSkills \u4E3B\u5BFC\u822A", children: [_jsx("div", { className: css.railLogo, children: _jsx(QuantSkillsBrandMark, { size: 38 }) }), _jsx("div", { className: css.railItems, children: NAV_ITEMS.map((item) => {
...
function PreferenceSaveState({ status, writable }) {
    const label = status === 'loading'
        ...
        : status === 'ready' && writable
            ? '外观更改会自动保存'
            : '当前连接不允许持久化设置';
    return _jsxs("p", { className: status === 'ready' && writable ? css.success : css.warning, role: "status", children: [status === 'ready' && writable ? _jsx(CheckCircle, {}) : _jsx(Clock, {}), label] });
}
