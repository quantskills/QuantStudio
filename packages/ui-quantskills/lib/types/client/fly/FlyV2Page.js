...
export default function FlyV2Page({ active, contest, openModelSettings, preparing = false, prepareMessage, onPrepare, tradePlans }) {
    ...
    const issueList = (value) => {
        ...
        if (!value.instruments.length || value.instruments.some(i => !/^[A-Za-z]{1,3}[0-9]{3,4}$/.test(i.symbol) || i.symbol.replace(/[0-9]+$/, '').toLowerCase() !== i.product.toLowerCase()))
    ...
    return _jsxs("div", { className: "fv-page", children: [_jsxs("header", { className: "fv-header", children: [_jsxs("div", { className: "fv-identity", children: [_jsx("div", { className: "fv-avatar", children: "\u2726" }), _jsxs("div", { children: [_jsx("div", { className: "fv-kicker", children: "QuantStudio \u00B7 \u671F\u8D27\u6A21\u62DF\u8D5B" }), _jsxs("h1", { children: [s.name, _jsx("span", { children: "\u679C\u8747\u4EA4\u6613\u5458" })] })] })] }), _jsxs("div", { className: "fv-header-actions", children: [_jsxs("span", { className: `fv-status ${running ? 'on' : ''}`, children: [_jsx("i", {}), s.control.trading ? (s.connection?.status === 'ready' ? '交易选择已启用' : '等待行情恢复') : s.neural.status === 'loading' ? '神经模型启动中' : running ? '个体运行中' : '已暂停'] }), _jsx("button", { type: "button", onClick: openSetup, children: "\u8BBE\u7F6E" }), tab === 'home' && _jsx("button", { type: "button", className: "fv-primary", disabled: pending, onClick: () => void run(() => control(s.control.paused ? 'start' : 'pause')), children: s.control.paused ? '唤醒小果' : '暂停个体' })] })] }), !s.environment.brain_ready && _jsxs("div", { className: "fv-runtime-note", role: "status", children: ["\u4E13\u7528 Python \u4E0E MaleCNS \u5C1A\u672A\u51C6\u5907\u3002", preparing ? prepareMessage : '打开设置，点击「一键准备果蝇」后自动下载并校验。', " ", _jsx("button", { type: "button", onClick: openSetup, children: "\u6253\u5F00\u73AF\u5883\u8BBE\u7F6E" })] }), _jsx("nav", { className: "fv-nav", "aria-label": "\u679C\u8747\u680F\u76EE", children: [['dashboard', '交易'], ['analysis', '交易分析'], ['home', '生活'], ['talk', '交流']].map(([key, label]) => _jsx("button", { type: "button", className: tab === key ? 'selected' : '', onClick: () => setTab(key), children: label }, key)) }), error && _jsxs("div", { role: "alert", className: "fv-error", children: [error, _jsx("button", { type: "button", onClick: () => setError(''), children: "\u5173\u95ED" })] }), s.neural.message && _jsx("p", { className: "fv-runtime-note", children: s.neural.message }), tab === 'dashboard' && _jsxs(_Fragment, { children: [_jsxs("div", { className: "fv-trading-bar", children: [_jsxs("div", { children: [_jsx("strong", { children: s.settings.life_validation ? '交易选择已关闭' : missing.length ? '待完成交易配置' : s.control.trading ? (s.control.close_only ? '仅生成平仓计划' : s.connection?.status === 'ready' ? '交易选择已启用' : '等待比赛行情恢复') : '交易建议已暂停' }), _jsx("span", { children: "\u751F\u6210\u8BA1\u5212\u540E\uFF0C\u7531\u4F60\u9010\u7B14\u786E\u8BA4\u4EA4\u6613\u3002" })] }), _jsxs("div", { className: "fv-actions", children: [_jsx("button", { type: "button", onClick: () => configureAt(2), children: "\u884C\u60C5\u914D\u7F6E" }), _jsx("button", { type: "button", className: "fv-primary", disabled: pending, onClick: () => { if (!s.control.trading && !canStart)
                                            configureAt(s.settings.life_validation ? 0 : missing[0].step);
                                        else
                                            void run(() => control(s.control.trading ? 'observe' : 'trade')); }, children: s.control.trading ? '暂停交易建议' : canStart ? '开始生成待确认计划' : '继续配置' })] })] }), !s.settings.life_validation && !!missing.length && _jsxs("div", { className: "fv-setup-needed", role: "status", children: [_jsx("span", { children: "\u5B8C\u6210\u4EE5\u4E0B\u8BBE\u7F6E\u540E\u5F00\u59CB\u751F\u6210\u4EA4\u6613\u8BA1\u5212" }), _jsx("div", { children: missing.map(issue => _jsxs("button", { type: "button", onClick: () => configureAt(issue.step), children: [issue.label, " \u2197"] }, issue.key)) })] }), _jsxs("section", { className: "fv-trading-section", "aria-label": "\u6210\u4EA4\u4E0E\u76C8\u4E8F", children: [_jsxs("header", { children: [_jsx("h2", { children: "\u6210\u4EA4\u4E0E\u76C8\u4E8F" }), _jsx("span", { children: "\u67DC\u53F0\u786E\u8BA4\u6210\u4EA4 \u00B7 \u5F53\u524D\u6301\u4ED3" })] }), _jsx(TradeStatistics, {}), tradePlans] }), _jsxs("section", { className: "fv-trading-section", "aria-label": "\u51B3\u7B56\u539F\u56E0", children: [_jsxs("header", { children: [_jsx("h2", { children: "\u51B3\u7B56\u539F\u56E0" }), _jsxs("span", { children: [s.settings.trade_period_minutes || 1, " \u5206\u949F\u51B3\u7B56"] })] }), _jsx(TradeLoop, { markets: s.markets, observing: !s.control.trading }), s.control.trading && _jsx("button", { type: "button", disabled: pending, onClick: () => void run(() => control(s.control.close_only ? 'trade' : 'close_only')), children: s.control.close_only ? '恢复开仓' : '本轮只平仓' }), _jsxs("details", { className: "fv-data-details", children: [_jsxs("summary", { children: ["\u884C\u60C5\u4E0E\u8FD0\u884C\u8BE6\u60C5", s.connection?.status === 'ready' ? ' · 已连接' : ' · 等待连接'] }), _jsx(RuntimeContinuity, { status: s }), _jsxs("div", { className: "fv-runtime-note", role: "status", "aria-label": "\u6BD4\u8D5B\u884C\u60C5\u8FDE\u63A5\u72B6\u6001", children: [_jsx("strong", { children: s.connection?.status === 'ready' ? '比赛行情已连接' : s.connection?.status === 'connecting' ? '正在连接比赛行情' : s.connection?.status === 'waiting' ? '行情暂不可用 · 自动恢复中' : '比赛行情未连接' }), s.connection?.message && _jsx("p", { children: s.connection.message }), connectionCooldown > 0 && _jsxs("p", { children: ["\u51B7\u5374\u4E2D\uFF0C", connectionCooldown, " \u79D2\u540E\u81EA\u52A8\u91CD\u8BD5\u3002"] }), s.connection?.updated_at && _jsxs("small", { children: ["\u6700\u8FD1\u540C\u6B65\uFF1A", new Date(s.connection.updated_at * 1000).toLocaleString('zh-CN', { hour12: false })] }), !['ready', 'waiting'].includes(s.connection?.status || '') && _jsx("button", { type: "button", disabled: pending || s.connection?.status === 'connecting' || connectionCooldown > 0 || !s.settings.instruments.length, onClick: () => void run(async () => {
