...
`SpawnTeammateRequest.provider` 选择 continuable spawn 或 fork provider。可选的 `agentOptions` 则独立选择 child Agent 的对话 provider、模型及其他 Agent 运行选项。`agentOptions` 中每个省略的字段都会继承 Team Lead 已解析的值，因此调用方可以覆盖单个 teammate 的模型，而不改变其他成员的默认行为。
...
每次 peer 投递都会把发送者前缀与消息内容加入 target 历史。任务和 roster 变更不增加模型 token；其面向模型的呈现属于 `@deepseek-ai/dsh-tool-agent-team` 结果。
