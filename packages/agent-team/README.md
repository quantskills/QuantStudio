...
`SpawnTeammateRequest.provider` selects the continuable spawn or fork provider. Its optional `agentOptions` separately selects the child Agent's conversation provider, model, and other Agent runtime options. Each omitted `agentOptions` field inherits the Team Lead's resolved value, so callers can override one teammate's model without changing the default behavior for other members.
...
Each peer delivery adds the sender prefix plus message content to the target history. Task and roster mutations add no model tokens; their model-facing representation belongs to `@deepseek-ai/dsh-tool-agent-team` results.
