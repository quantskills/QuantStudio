...
export function computeColumns(
  ...
  // A mounted right-side workbench owns a visible split rather than an
  // optional details drawer. Keep its reservation (and any wider native
  // preference) while the center absorbs the remaining width. The caller can
  // read the resolved track through AppFrame's CSS variable, so even a
  // viewport smaller than both reservations stays geometrically aligned.
  if (reservedDetails > 0) {
    const resolvedDetails = Math.min(
      Math.max(d0, reservedDetails),
      Math.max(0, viewport - s),
    )
    return { sidebar: s, center: Math.max(0, viewport - s - resolvedDetails), details: resolvedDetails }
  }
