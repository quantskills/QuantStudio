...
侧栏 **比赛** 提供独立的 PandaAI 期货仿真比赛工作台，默认关闭。开启并在官网授权后，可查看资金、持仓、委托、成交和排名。进入 AI 交易助手会继续该账户主对话，自动巡检账户，并提供持仓研究、指定品种研究和今日复盘；单笔交易通过计划卡确认。比赛提示词、技能和工具限制仅作用于比赛会话。关闭后普通会话继续使用；已提交的委托仍由赛事柜台处理。[查看接入与使用说明](docs/contest.md)。
